...
from plugins.upr.i18n import parse_export_language
from plugins.upr.service import UprVisualsService
from plugins.upr.typography import export_style_token

logger = logging.getLogger(__name__)

ASSIGNMENT_EXPORT_JOB_TYPE = "upr.assignment_export"
...
def serve_assignment_export(job_id: str, *, aes_id: int, as_attachment: bool = True):
    ...
    filename = str(meta.get("filename") or "UPR.pdf")
    if not raw_path:
        raise NotFound("Export is not ready.")
    path = Path(raw_path)
