...
(function() {
    ...
    document.addEventListener('click', async function(e) {
        ...
        try {
            ...
            const encodedId = (typeof _encodeWorkflowId === 'function') ? _encodeWorkflowId(workflowId) : btoa(workflowId).replace(/\+/g, '-').replace(/\//g, '_').replace(/=/g, '');
            if (apiFn) {
                data = await apiFn(`/api/ai/documents/workflows/${encodedId}`);
            } else {
                const fn = (window.getFetch && window.getFetch()) || fetch;
                const r = await fn(`/api/ai/documents/workflows/${encodedId}`, { credentials: 'same-origin' });
