...
def classify_and_rewrite_user_message(
    ...
    Substantive queries are returned verbatim (user's own words). Unified preflight
    handles greeting/scope classification. Follow-up expansion is LLM-driven via
    prompts — no regex rewrite shortcuts (see ``AgentRoutingPolicy``).
    """
    raw = (message or "").strip()
    if not raw:
        return raw, False, True
