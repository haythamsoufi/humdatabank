...
describe('matrix-field-chunking', () => {
  ...
  it('finds matrix hidden inputs but not other field types', () => {
    const form = buildForm(`
      ${matrixBlock(1, 'b64:aGVsbG8=')}
  ...
  it('excludes disabled matrix inputs', () => {
    const form = buildForm(`
      <div class="form-item-block" data-item-type="matrix">
        <input type="hidden" name="field_value[1]" value="b64:aGVsbG8=" disabled>
      </div>
    `);
  ...
  it('does not chunk non-matrix hidden fields even if large', () => {
    const bigValue = 'b64:' + 'A'.repeat(900);
    const form = buildForm(`
      <div class="form-item-block" data-item-type="question" data-question-type="textarea">
        <input type="hidden" name="field_value[9]" value="${bigValue}">
      </div>
    `);
    chunkLargeMatrixFields(form);
    const input = form.querySelector('input[name="field_value[9]"]');
