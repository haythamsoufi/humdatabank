...
| Login works, first country/entity pick returns to `/login` with “session needed a refresh / changes were not saved” (phones) | CSRF failed on the context-switch POST *and* the handler replaced the login cookie with a fresh anonymous one, so the recoverable case became a real logout | Context switch is GET (no CSRF token to go stale). An unauthenticated CSRF failure must not emit a replacement session cookie — `g.suppress_session_cookie` stops `save_session` — so a browser that merely omitted the cookie can present it on the next request and stay logged in. |
