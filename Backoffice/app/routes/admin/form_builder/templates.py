...
def _shared_users_for_template_list(template):
    ...
    seen = set()
    users = []
    for share in getattr(template, "shared_with", None) or []:
        u = getattr(share, "shared_with_user", None)
        if not u or u.id in seen:
            continue
        if owner_id is not None and u.id == owner_id:
            continue
        seen.add(u.id)
        users.append(u)
...
def manage_templates():
    ...
    template_shared_with_users = {t.id: _shared_users_for_template_list(t) for t in templates}
...
def edit_template(template_id):
    ...
    template_shared_with_users = (
        db.session.query(User)
        .join(TemplateShare, TemplateShare.shared_with_user_id == User.id)
        .filter(TemplateShare.template_id == template.id)
