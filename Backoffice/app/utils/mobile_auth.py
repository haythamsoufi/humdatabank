...
def _try_jwt_auth() -> bool:
    ...
    try:
        ...
        current_app.logger.debug("_try_jwt_auth: JWT decode failed: %s", _jwt_exc)
        return False
    ...
    if not user or not user.is_active:
        current_app.logger.debug(
            "_try_jwt_auth: user not found or inactive (user_id=%s, found=%s)",
            claims.user_id, user is not None,
        )
        return False
