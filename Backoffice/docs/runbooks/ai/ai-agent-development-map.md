...
Where agent behavior lives besides the orchestrator so new work lands in the right module and **`app/services/ai_agent_executor.py`** stays focused on the tool loop.

## Role of `ai_agent_executor.py`

`app/services/ai_agent_executor.py` is primarily responsible for:
...
- delegating policy, formatting, and inference to the modules below.

## Module boundaries

| Module | Responsibility | Main public functions | Typical callers |
|---|---|---|---|
...
| `app/services/ai_fastpaths/unified_plans_focus_fastpath.py` | Deterministic fast path for unified-plans review | `run_unified_plans_focus_fastpath()` | `ai_agent_executor.py` |

## Where to put new behavior

Use this routing rule when extending the agent:
...
- orchestration / loop / lifecycle only → `ai_agent_executor.py`

## PR guardrails

- Avoid dumping large regex or prompt blocks into `ai_agent_executor.py`; add helpers in the owning module and import them.
- Prefer module-level units (policy, inference, fast path) so changes stay testable before wiring into the executor.
