...
class AdminNotifications {
    ...
    init() {
        ...
        document.getElementById('retry-all-failed-emails')?.addEventListener('click', () => {
            this.retryAllFailedEmails();
        });

        document.getElementById('dismiss-all-failed-emails')?.addEventListener('click', () => {
            this.dismissAllFailedEmails();
        });

        document.getElementById('retry-selected-failed-emails')?.addEventListener('click', () => {
            this.retrySelectedFailedEmails();
        });
    ...
    _initNotificationsCenterTabs() {
        ...
        if (!A) {
            console.warn('AdminUnderlineTabs missing; ensure layout loads js/admin/underline-tabs.js before communication.js');
            return;
        }
        const listSel = '#communication-center-tabs';
    ...
    switchTab(tab) {
        ...
        const A = window.AdminUnderlineTabs;
        if (A) {
            A.activateStripTab('#communication-center-tabs', tab, { panelSelector: '.communication-center-panel' });
        }
    ...
    async retryEmailDelivery(logId, triggerButton) {
        ...
        if (btn) {
            btn.disabled = true;
            btn.innerHTML = `<i class="fas fa-spinner fa-spin fa-fw"></i>`;
        }
        try {
            const data = await _anFetch(`/admin/api/communications/email-delivery/${encodeURIComponent(logId)}/retry`, {
                method: 'POST',
            });
            if (data.success) {
                if (window.showAlert) {
            ...
            if (window.showAlert) {
                window.showAlert('An error occurred while retrying the email.', 'error');
            }
        } finally {
            if (btn) {
                btn.disabled = false;
    ...
    getSelectedEmailLogIds() {
        ...
        if (manager && typeof manager.getSelectedEmailLogIds === 'function') {
            return manager.getSelectedEmailLogIds();
        }
        return [];
    }

    updateGridEmailSelectionToolbar() {
        const toolbar = document.getElementById('grid-email-selection-toolbar');
        const countEl = document.getElementById('grid-email-selection-count');
        if (!toolbar || !countEl) {
            return;
        }
        const logIds = this.getSelectedEmailLogIds();
        if (!logIds.length) {
        ...
        countEl.textContent = String(logIds.length);
    }

    clearGridEmailSelection() {
        const manager = window.communicationsGridManager || window.notificationsGridManager;
        if (manager && typeof manager.clearEmailRowSelection === 'function') {
            manager.clearEmailRowSelection();
        }
        this.updateGridEmailSelectionToolbar();
    }

    async cancelEmailDelivery(logId, triggerButton) {
        const t = window.COMMUNICATION_TRANSLATIONS || {};
        const btn = triggerButton;
        const originalHtml = btn ? btn.innerHTML : null;
        if (btn) {
        ...
        try {
            const data = await _anFetch(`/admin/api/communications/email-delivery/${encodeURIComponent(logId)}/cancel`, {
                method: 'POST',
            });
            if (data.success) {
                if (window.showAlert) {
            ...
            if (window.showAlert) {
                window.showAlert('An error occurred while dismissing the email failure.', 'error');
            }
        } finally {
            if (btn) {
                btn.disabled = false;
    ...
    async dismissAllFailedEmails() {
        const t = window.COMMUNICATION_TRANSLATIONS || {};
        if (!window.confirm(t.confirmDismissAll || 'Dismiss all failed emails without retrying?')) {
            return;
        }
        const button = document.getElementById('dismiss-all-failed-emails');
        const originalHtml = button ? button.innerHTML : null;
        if (button) {
            button.disabled = true;
            button.innerHTML = `<i class="fas fa-spinner fa-spin mr-2"></i>${t.dismissingEmail || 'Dismissing...'}`;
        }
        try {
            const data = await _anFetch('/admin/api/communications/email-delivery/cancel-failed', {
                method: 'POST',
                body: JSON.stringify({}),
            });
            if (window.showAlert) {
            ...
            if (window.showAlert) {
                window.showAlert('An error occurred while dismissing failed emails.', 'error');
            }
        } finally {
            if (button) {
    ...
    async retrySelectedFailedEmails() {
        const logIds = this.getSelectedEmailLogIds();
        if (!logIds.length) {
            return;
        }
        await this._bulkEmailDeliveryAction('retry', logIds, 'retry-selected-failed-emails');
    }

    async dismissSelectedFailedEmails() {
        const t = window.COMMUNICATION_TRANSLATIONS || {};
        const logIds = this.getSelectedEmailLogIds();
        if (!logIds.length) {
            return;
        }
        if (!window.confirm(t.confirmDismissSelected || 'Dismiss selected email failure(s) without retrying?')) {
            return;
        }
        await this._bulkEmailDeliveryAction('cancel', logIds, 'dismiss-selected-failed-emails');
    }
