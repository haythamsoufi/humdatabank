...
from plugins.upr.loaders import _load_entries, _load_items
...
from plugins.upr.pns_funding import _find_country_aes_for_year, _load_t23_pns_totals
    ...
    *,
    ...
    if country_id and year:
        try:
            ...
            logger.exception("UPR: failed to load T24 funding requirements for country %s year %s", country_id, year)
    if host_ns_id and period_name:
        try:
            pns_reported = _load_t23_pns_totals(host_ns_id, period_name, year)
        except Exception:
            logger.exception("UPR: failed to load T23 PNS funding for host NS %s", host_ns_id)
