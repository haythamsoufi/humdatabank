...
export class TemplateVariablesManager {
    ...
    async saveVariables() {
        try {
            ...
            // WAF: wrap body in { payload: b64 } so Azure OWASP CRS rules don't inspect
            // variable display names / default values for SQL/XSS patterns.
            // get_request_data() unwraps { payload: b64 } transparently on the server.
            const _varPayloadB64 = btoa(unescape(encodeURIComponent(JSON.stringify({ variables: this.variables }))));
