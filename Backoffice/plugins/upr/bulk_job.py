"""UPR bulk export as a Track A job on AIJob / ai_job_runner.
...
from plugins.upr.audience import apply_narrative_audience, resolve_narrative_audience
from plugins.upr.bulk import match_narrative_path, normalize_export_format
from plugins.upr.catalog import dashboards_for_kind, kind_for_template
from plugins.upr.data import build_payload, filename_from_visual_title
from plugins.upr.i18n import export_locale, parse_export_language
from plugins.upr.render import render_dashboard_html
from app.utils.pg_advisory_lock import acquire_transaction_advisory_lock
from plugins.upr.service import STORAGE_CATEGORY, UprVisualsService
...
BULK_EXPORT_JOB_TYPE = "upr.bulk_export"
...
def _job_dir(job_id: str) -> Path:
    return Path(current_app.instance_path) / "upr_tmp" / str(job_id)
...
def _fail_orphaned_bulk_jobs() -> None:
    ...
    for existing in jobs:
        ...
        db.session.commit()
        logger.warning("Failed orphaned UPR export: job=%s", job_id)
...
def create_bulk_export_job(
    *,
    ...
    from plugins.upr.data import get_assigned_form_for_bulk
    ...
    if inflight:
        raise RuntimeError("A UPR export is already running.")
...
def request_bulk_export_cancel(job_id: str) -> str:
    ...
    try:
        ...
        logger.debug("UPR cancel item update failed: job=%s err=%s", job_id, exc)
        db.session.rollback()
    meta = dict(job.meta or {})
            ...
            for job in expired:
                ...
                if zip_key and isinstance(zip_key, str):
                    try:
                        ...
                        logger.debug("UPR zip cleanup failed: job=%s", job.id, exc_info=True)
                session.delete(job)
            if expired:
                logger.info("Cleaned up %s expired UPR export jobs", len(expired))
    except Exception:
        logger.exception("Failed to clean up expired UPR export jobs")
...
def _update_job_progress(job_id: str, item_id: int | None = None, *, force: bool = False, **fields: Any) -> None:
    ...
    try:
        ...
        logger.exception("Failed to persist UPR job progress %s", job_id)


def _process_bulk_export_item_sync(app, *, job_id: str, item_id: int) -> None:
    work_root = Path(app.instance_path) / "upr_tmp"
    ...
    with app.app_context():
        ...
        try:
            ...
            from plugins.upr.data import list_countries_for_bulk
            ...
            with zipfile.ZipFile(zip_path, "w", zipfile.ZIP_DEFLATED) as zf, export_locale(lang):
                for row in assignments:
                    ...
                    try:
                        ...
                        logger.warning("UPR skip aes %s: %s", row["aes_id"], exc)
                    ...
                    for dashboard_id in dashboards:
                        ...
                        try:
                            ...
                            logger.warning(
                                "UPR render timed out for aes %s dashboard %s",
                                row["aes_id"],
                                dashboard_id,
                            )
            ...
            zip_key = f"exports/{job_id}/upr.zip"
            ...
            logger.exception("UPR bulk export failed")
            _update_job_progress(
            ...
            try:
                ...
                db.session.rollback()
                logger.exception("UPR job item commit failed: job=%s", job_id)
