...
class TestOrgRoutes:
    ...
    def test_branches_returns_list(self, client, admin_jwt_headers, db_session):
        resp = client.get(f'{PREFIX}/admin/org/branches/1', headers=admin_jwt_headers)
        assert_mobile_ok(resp, has_data=True)
        assert 'branches' in resp.get_json()['data']

    def test_subbranches_returns_list(self, client, admin_jwt_headers, db_session):
        resp = client.get(f'{PREFIX}/admin/org/subbranches/1', headers=admin_jwt_headers)
        assert_mobile_ok(resp, has_data=True)
