"""Export-language helpers for UPR (MT + Backoffice locale).
...
def can_machine_translate(lang: str | None = None) -> bool:
    ...
    try:
        ...
        logger.warning("UPR: translation engine unavailable; skipping live MT", exc_info=True)
        return False
...
def _progress_dir() -> Path | None:
    try:
        ...
        if has_app_context():
            path = Path(current_app.instance_path) / "upr_progress"
            path.mkdir(parents=True, exist_ok=True)
            return path
    except Exception:
        logger.debug("UPR: progress directory unavailable", exc_info=True)
    return None
...
def _write_progress_file(key: str, rec: dict[str, Any]) -> None:
    ...
    try:
        ...
        logger.debug("UPR: could not persist progress %s", key, exc_info=True)
        try:
            tmp.unlink()
        except OSError:
            pass
...
def t(text: str | None) -> str:
    ...
    if session and session.get("phase") == "apply":
        ...
        if hit:
            return str(hit)
    from plugins.upr.strings import lookup_visual_string
    ...
    if is_system_language(lang):
        try:
            ...
            logger.debug("UPR gettext failed for %r → %s", raw[:80], lang, exc_info=True)
            return raw
    ...
    try:
        ...
        logger.debug("UPR t() failed for %r → %s", raw[:80], lang, exc_info=True)
    return raw
...
def localized_ns_display_name(ns, fallback: str = "") -> str:
    ...
    from plugins.upr.catalog import display_ns_name
