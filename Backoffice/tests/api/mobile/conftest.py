...
def admin_mobile_user(db_session, app):
    """Mobile user with admin_core role and all standard admin permissions."""
    return create_test_admin(
        db_session, email='admin-mobile@test.com', password='AdminPass123!',
    )
...
def _make_jwt_headers(app, user, session_id=None):
    # Use a unique session_id per call so that a logout in one test cannot blacklist
    # the hardcoded 'test-sid' string and poison all subsequent tests in the session.
...
def admin_jwt_headers(app, admin_mobile_user):
    ...
    return _make_jwt_headers(app, admin_mobile_user)


@pytest.fixture
def sm_jwt_headers(app, sm_mobile_user):
    """Valid JWT headers for a system manager."""
    return _make_jwt_headers(app, sm_mobile_user)
