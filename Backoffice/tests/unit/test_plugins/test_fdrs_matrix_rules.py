...
_PATCH_CHECK = "plugins.fdrs.validation.fdrs_matrix.rules.ValidationKpiCheckType"
_PATCH_THRESH = "plugins.fdrs.validation.fdrs_matrix.rules.ValidationThreshold"
_PATCH_CYR = "plugins.fdrs.validation.fdrs_matrix.rules.CountryYearReference"
_PATCH_ATTR = "plugins.fdrs.validation.fdrs_matrix.rules.CountryAttribute"
...
class TestRunFdrsMatrixRulesDeaths:
    ...
    def test_volunteer_deaths_fires_when_gte_1(
        ...
        with patch("plugins.fdrs.validation.fdrs_matrix.rules.numeric_value",
    ...
    def test_deaths_not_fired_when_zero(
        ...
        with patch("plugins.fdrs.validation.fdrs_matrix.rules.numeric_value", return_value=0.0):
            ctx = _make_ctx(kpi_data={"KPI_noVolDeathsDuty_Tot": (entry, item)})
            results = run_fdrs_matrix_rules(ctx)
...
class TestRunFdrsMatrixRulesBranchesHigherUnits:
    ...
    def test_branches_higher_units_fires(
        ...
        with patch("plugins.fdrs.validation.fdrs_matrix.rules.numeric_value", side_effect=_nv), \
             patch("plugins.fdrs.validation.fdrs_matrix.rules.is_reported_value", return_value=True):
...
class TestRunFdrsMatrixRulesPopulation:
    ...
    def test_higher_than_pop_fires(
        ...
        with patch("plugins.fdrs.validation.fdrs_matrix.rules.numeric_value", return_value=2_000_000), \
             patch("plugins.fdrs.validation.fdrs_matrix.rules.is_reported_value", return_value=True):
    ...
    def test_significant_pop_fires(
        ...
        with patch("plugins.fdrs.validation.fdrs_matrix.rules.numeric_value", return_value=400_000), \
             patch("plugins.fdrs.validation.fdrs_matrix.rules.is_reported_value", return_value=True):
...
class TestRunFdrsMatrixRulesGrbmp:
    ...
    def test_grbmp_fires_when_migration_zero(
        ...
        with patch("plugins.fdrs.validation.fdrs_matrix.rules.numeric_value", return_value=0), \
             patch("plugins.fdrs.validation.fdrs_matrix.rules.is_reported_value", return_value=True):
...
class TestRunFdrsMatrixRulesFiscalYear:
    ...
    def test_fiscal_year_fires_when_over_365_days(
        ...
        with patch("plugins.fdrs.validation.fdrs_matrix.rules.numeric_value", return_value=400.0), \
             patch("plugins.fdrs.validation.fdrs_matrix.rules.is_reported_value", return_value=True):
...
class TestRunFdrsMatrixRulesMissingDocuments:
    ...
    def test_missing_ar_fires_when_no_submitted_doc(
        ...
        with patch("plugins.fdrs.validation.fdrs_matrix.rules.numeric_value", return_value=None), \
             patch("plugins.fdrs.validation.fdrs_matrix.rules.is_reported_value", return_value=False):
            ctx = _make_ctx(kpi_data={})
            results = run_fdrs_matrix_rules(ctx)
...
class TestRunFdrsMatrixRulesSimilarIndigenousReachNonNumeric:
    ...
    def test_non_numeric_ind_val_is_silently_skipped(self):
        ...
        with patch(_PATCH_FI) as mock_fi, patch(_PATCH_SD) as mock_sd, patch(
            ...
            "plugins.fdrs.validation.fdrs_matrix.rules.is_reported_value",
            return_value=True,
        ), patch(
            "plugins.fdrs.validation.fdrs_matrix.rules.numeric_value",
            return_value=100.0,
        ):
