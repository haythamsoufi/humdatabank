...
- Because msgids come from extraction and not from the database, adding a new `_()` call in Python or Jinja means running `python scripts/i18n/extract_update_translations.py` (or **Regenerate Strings** on `/admin/translations/manage`) and committing the catalog. Backoffice CI blocks on `scripts/ci/check_translations_current.py` if you do not — the string cannot be translated in the admin grid until the catalog lists it. `Babel` is pinned in `requirements.txt` so regeneration does not rewrite every `.po` header.
