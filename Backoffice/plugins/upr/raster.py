...
def _font_css() -> str:
    from plugins.upr.typography import export_font_face_css

    return export_font_face_css()


def _document_body_font_css(lang: str | None = None) -> str:
    from plugins.upr.typography import document_root_font_css
...
def _rtl_print_css(lang: str | None = None) -> str:
    ...
    Screen preview uses the same ``typography_css()`` via ``/upr/fonts.css``.
    ...
    """
    from plugins.upr.i18n import is_rtl
...
def _pdf_page_css(dashboard_id: str) -> str:
    ...
    from plugins.upr.i18n import current_export_language, rtl_css
...
def _ns_logo_data_uri(filename: str) -> str:
    ...
    try:
        ...
        logger.debug("UPR: could not load NS logo %s for export", name, exc_info=True)
        return ""
    if not data:
        return ""
...
def _fetch_remote_as_data_uri(url: str) -> str:
    ...
    try:
        ...
        logger.debug("UPR: could not fetch remote image %s for export", url, exc_info=True)
        return ""
    if not data:
        return ""
    if len(data) > _MAX_REMOTE_IMAGE_BYTES:
        logger.warning("UPR: remote image exceeded %s bytes: %s", _MAX_REMOTE_IMAGE_BYTES, url)
        return ""
    mime = mimetypes.guess_type(name)[0] or "image/png"
    return f"data:{mime};base64,{base64.b64encode(data).decode('ascii')}"
...
def render_png_isolated(
    ...
    *,
    ...
    from plugins.upr.i18n import current_export_language

    env["UPR_VISUALS_LANG"] = current_export_language()
    cmd = [
        sys.executable,
        "-c",
        "from plugins.upr.raster import render_png_job_file; "
