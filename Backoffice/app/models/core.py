...
class User(UserMixin, db.Model):
    ...
    def set_password(self, password):
        try:
            ...
            ph = PasswordHasher()
            self.password_hash = 'argon2:' + ph.hash(password)
        except ImportError:
            # Fallback to PBKDF2-SHA256 if argon2-cffi is not installed
            self.password_hash = generate_password_hash(password, method='pbkdf2:sha256')

    def check_password(self, password):
        if self.password_hash is None:
            return False
        # Handle argon2id hashes created by set_password() after the argon2-cffi migration
        if isinstance(self.password_hash, str) and self.password_hash.startswith('argon2:'):
            try:
                from argon2 import PasswordHasher
                from argon2.exceptions import VerifyMismatchError, VerificationError, InvalidHashError
                ph = PasswordHasher()
                raw_hash = self.password_hash[len('argon2:'):]
                try:
                    ph.verify(raw_hash, password)
                    if ph.check_needs_rehash(raw_hash):
                ...
                current_app.logger.error("argon2-cffi not installed; cannot verify argon2 password hash")
                return False
...
class UserSessionLog(db.Model):
    ...
    # Histogram of canonical path keys (resolved request path; mobile /m/...); see app.utils.page_view_paths
