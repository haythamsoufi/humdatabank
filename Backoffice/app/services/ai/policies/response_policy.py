...
- explicit user table-request detection (output formatting)
- output sanitization for end-user safety/UX

Tool routing is LLM-driven — see ``agent_routing.AgentRoutingPolicy``.
"""
...
_TOP_BOTTOM_BLOCK_RE = re.compile(
    r"(?is)(?:^|\n)(?:Summary\s*[—\-:].*?\n)?"
    r"(?:.*?(?:Top\s+\d+|Bottom\s+\d+).*?\n)+"
    r"(?:.*?(?:Aggregate|Regional pattern|Caveats?).*?\n)*"
)


def _strip_matrix_share_verbose_prose(text: str) -> str:
    """Drop top/bottom country lists and long caveat blocks when the UI table shows rows."""
    if not text:
        return text
    text = _TOP_BOTTOM_BLOCK_RE.sub("\n", text)
    text = re.sub(
        r"(?is)\n*(?:Caveats?(?:\s+and\s+interpretation)?|Regional pattern)\s*:.*?(?=\n## Sources|\Z)",
        "\n",
        text,
    )
    text = re.sub(r"(?im)^Note: Iraq appears in both lists.*\n?", "", text)
    return re.sub(r"\n{3,}", "\n\n", text).strip()
...
def sanitize_agent_answer(text: str, *, has_table_payload: bool = False, table_kind: Optional[str] = None) -> str:
    ...
    if has_table_payload:
        ...
        kind = str(table_kind or "").strip().lower()
        if kind == "indicator_search":
            text = _strip_indicator_search_bullets(text)
        elif kind == "matrix_share":
            text = _strip_matrix_share_verbose_prose(text)
