...
| Admin UI | `/admin/fdrs-tools#publication` (legacy `/admin/plugins/fdrs/publication` redirects here) — pick one assignment (reporting period), preview additions/changes/removals per country with a diff highlighted per item, then publish all countries or a selected subset. Progress and cancel stay in the publication panel. |
