...
from .enums import AssignmentEntityStatusValue, AssignmentSectionStatusValue, PublicSubmissionStatus
...
class AssignmentEntityStatus(db.Model):
    ...
    # Who last set *any* status (pending, in progress, submitted, approved, …)
    ...
    __table_args__ = (
        ...
        db.Index('ix_aes_status_changed_by', 'status_changed_by_user_id'),
...
class AssignmentPageStatus(db.Model):
    """Per-page workflow status for one assignment entity."""

    __tablename__ = 'assignment_page_status'
    ...
    form_page_id = db.Column(
        db.Integer,
        db.ForeignKey('form_page.id', ondelete='CASCADE'),
        nullable=False,
    )
    status = db.Column(
        db.String(20),
    ...
    __table_args__ = (
        ...
        db.Index('ix_aps_aes', 'assignment_entity_status_id'),
        db.Index('ix_aps_page', 'form_page_id'),
        db.Index('ix_aps_status', 'status'),
    )

    def __repr__(self):
        return (
            f'<AssignmentPageStatus AES:{self.assignment_entity_status_id} '
            f'Page:{self.form_page_id} Status:{self.status}>'
        )
