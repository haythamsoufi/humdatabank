 ...
 *
 * Indicators are loaded sequentially and the live DOM is resolved on each insert so
 * parallel fetch races and stale container references cannot drop saved indicators.
 */
    ...
    for (const item of items) {
        try {
            if (item.html) {
                ...
                if (data && data.html) {
                    _insertRepeatIndicatorHtml(sectionId, instanceNumber, data.html);
                }
            }
        } catch (err) {
