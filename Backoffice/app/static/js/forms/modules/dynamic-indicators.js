...
function exceedsMaximumLimit(sectionContainer, sectionId, maxIndicators) {
    ...
    if (currentIndicatorCount + pendingRowsCount >= parseInt(maxIndicators)) {
        ...
        else (window.__clientWarn || console.warn)(message);
        return true;
    }
    return false;
}
...
function deleteDynamicIndicator(assignmentId, indicatorName) {
    ...
    if (!csrfToken) {
        ...
        else (window.__clientWarn || console.warn)(errorMessage);
        return;
    }
    ...
    displayFilters.forEach(filterKey => {
        ...
        filterDropdownsHtml += `
            <div class="${hasOnlyOneFilter ? 'w-48' : 'flex-1 min-w-[200px]'}">
                ...
                <select class="w-full pl-3 pr-8 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-green-500 ${filterKey}-select"
                        data-row-id="${rowId}"
