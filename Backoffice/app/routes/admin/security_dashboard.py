...
def security_events():
    ...
    # Fetch all records — ag-grid handles client-side pagination itself,
    # so server-side pagination would silently truncate the dataset.
    all_events = query.all()

    # Wrap in a simple namespace so the template can use events.items and events.total
    class _EventsWrapper:
        def __init__(self, items):
            self.items = items
            self.total = len(items)

    events = _EventsWrapper(all_events)
