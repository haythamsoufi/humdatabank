...
class TestBaselineValue:
    ...
    def _fn(self, history, year, check_type):
        from plugins.fdrs.validation.fdrs_matrix.history import baseline_value
        return baseline_value(history, year, check_type)

    def test_past_year_returns_year_minus_1(self):
        from plugins.fdrs.validation.fdrs_matrix.history import CHECK_TYPE_PAST_YEAR
    ...
    def test_3year_avg_single_value(self):
        from plugins.fdrs.validation.fdrs_matrix.history import CHECK_TYPE_3YEAR_AVG
...
class TestYtdPct:
    ...
    def _fn(self, current, baseline):
        from plugins.fdrs.validation.fdrs_matrix.history import ytd_pct
...
class TestThresholdExceeded:
    ...
    def _fn(self, ytd, threshold):
        from plugins.fdrs.validation.fdrs_matrix.history import threshold_exceeded
...
def _run_fdrs_rules(ctx, numeric_side_effect=None, is_reported=True):
    ...
    with patch("plugins.fdrs.validation.fdrs_matrix.rules.ValidationKpiCheckType") as m1, \
         patch("plugins.fdrs.validation.fdrs_matrix.rules.ValidationThreshold") as m2, \
         patch("plugins.fdrs.validation.fdrs_matrix.rules.CountryYearReference") as m3, \
         patch("plugins.fdrs.validation.fdrs_matrix.rules.CountryAttribute") as m4, \
         patch("app.models.FormItem") as m5, \
         patch("app.models.SubmittedDocument") as m6, \
         patch("plugins.fdrs.validation.fdrs_matrix.rules.is_reported_value", return_value=is_reported), \
         patch(
...
class TestFdrsMatrixRulesConstants:
    ...
    def test_non_zero_kpi_codes_include_gb(self):
        from plugins.fdrs.validation.fdrs_matrix.rules import NON_ZERO_KPI_CODES
    ...
    def test_health_sub_kpi_codes_include_train_fa(self):
        from plugins.fdrs.validation.fdrs_matrix.rules import HEALTH_SUB_KPI_CODES
    ...
    def test_thematic_reach_includes_drr(self):
        from plugins.fdrs.validation.fdrs_matrix.rules import THEMATIC_REACH_FOR_TYPEOF
