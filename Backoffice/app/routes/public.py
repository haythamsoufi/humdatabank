...
def _check_dbinfo_access():
    ...
    enabled = current_app.config.get("ENABLE_DBINFO", False)
    ...
    # NOTE: rely on WSGI-layer ProxyFix (or equivalent) to set REMOTE_ADDR correctly
    # when behind a trusted reverse proxy. Do NOT read X-Forwarded-For here — it is
    # trivially spoofable by any client and must not be used for access control.
    if current_app.config.get("FLASK_CONFIG") == "production":
        if request.remote_addr not in ['127.0.0.1', '::1', 'localhost']:
            current_app.logger.warning(
                f"SECURITY: /dbinfo endpoint accessed from non-localhost IP: {request.remote_addr} by user {current_user.id}"
            )
            return (False, 403)
...
def _check_migrate_access():
    ...
    enabled = current_app.config.get("ENABLE_MIGRATE", False)
    if not enabled:
    ...
    # Prefer Authorization: Bearer <token> header; URL param is accepted for
    # backward compatibility but logs a deprecation warning (tokens in URLs
    # appear in server logs and browser history).
    token_env = os.environ.get("MIGRATE_TOKEN")
    token_req = request.headers.get("Authorization", "").removeprefix("Bearer ").strip() or None
    url_token = request.args.get("token")
    if url_token and not token_req:
        current_app.logger.warning(
            "SECURITY: /migrate token supplied as URL query param — use "
            "'Authorization: Bearer <token>' header instead; URL tokens are logged."
        )
        token_req = url_token
