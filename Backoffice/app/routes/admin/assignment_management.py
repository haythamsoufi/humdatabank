...
def manage_assignments():
    ...
    if is_json_request():
        ...
        for assignment in assignments:
            ...
            public_submission_count = submission_counts.get(assignment.id, 0) if hasattr(assignment, 'public_submissions') else None
...
def new_assignment():
    ...
    if form.validate_on_submit():
        try:
            ...
            return redirect(url_for("assignment_management.edit_assignment", assignment_id=new_assignment.id))
        except Exception as e:
...
def generate_public_url(assignment_id):
    ...
    next_url = get_safe_redirect_url(request.form.get('next'), 'assignment_management.manage_assignments')
