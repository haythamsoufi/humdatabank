...
def list_assignments():
    """List form assignments (paginated)."""
    from app.models import AssignedForm, PublicSubmission
    from app.utils.api_pagination import validate_pagination_params

    page, per_page = validate_pagination_params(request.args, default_per_page=50, max_per_page=200)

    paginated = AssignedForm.query.options(
        db.joinedload(AssignedForm.template),
    ).order_by(AssignedForm.period_name.desc()).paginate(page=page, per_page=per_page, error_out=False)

    assignment_ids = [a.id for a in paginated.items]
    submission_counts: dict = {}
    if assignment_ids:
        rows = db.session.query(
            PublicSubmission.assigned_form_id,
            db.func.count(PublicSubmission.id).label('cnt'),
        ).filter(
            PublicSubmission.assigned_form_id.in_(assignment_ids)
        ).group_by(PublicSubmission.assigned_form_id).all()
        submission_counts = {row.assigned_form_id: row.cnt for row in rows}
    ...
    for a in paginated.items:
        items.append({
            ...
            'public_submission_count': submission_counts.get(a.id, 0),
        })
...
def list_indicators():
    ...
    sectors_dict = _get_sector_dict()
    subsectors_dict = _get_subsector_dict()
...
@mobile_auth_required(permission='admin.indicator_bank.archive')
...
def update_translation(translation_id):
    ...
    PO-file based in-place updates are not yet implemented via the mobile API.
    Use the Backoffice web UI (/admin/translations) to edit translations.
    """
    return mobile_error(
        'Translation updates via the mobile API are not yet implemented. '
        'Use the Backoffice web interface to edit translations.',
        501,
