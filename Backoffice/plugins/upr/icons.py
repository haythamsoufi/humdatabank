"""SPEF icon and National Society logo resolution for UPR."""
...
def _set_spef_icon_mode(*, inline: bool) -> None:
    try:
        ...
        logger.debug("UPR: could not set SPEF icon mode", exc_info=True)
...
def _inline_spef_icons() -> bool:
    try:
        ...
        logger.debug("UPR: could not read SPEF icon mode", exc_info=True)
    return False
...
def _spef_catalog_icon_url(row) -> str:
    ...
    try:
        ...
        logger.debug("UPR: SPEF catalog icon URL via API failed", exc_info=True)
        try:
            return spef_icon_url(row) or ""
        except Exception:
            logger.debug("UPR: SPEF catalog icon URL fallback failed", exc_info=True)
            return ""
...
def _inline_local_icon(path: Path) -> str:
    ...
    try:
        ...
        logger.debug("UPR: could not inline local icon %s", path, exc_info=True)
        return ""
...
def _plugin_reach_icon_src(code: str, *, inline: bool) -> str:
    ...
    try:
        ...
        return url_for("upr.static_file", filename=rel)
    except Exception:
        logger.debug("UPR: could not build reach icon URL for %s", rel, exc_info=True)
        return f"/upr/static/{rel}"
...
def _inline_spef_icon(row) -> str:
    ...
    try:
        ...
        logger.debug("UPR: could not inline SPEF icon %s", filename, exc_info=True)
        return ""
...
def _inline_ns_logo(filename: str) -> str:
    ...
    try:
        ...
        logger.debug("UPR: could not inline NS logo %s", name, exc_info=True)
        return ""
...
def _ns_logo_src(ns, iso3: str, *, inline: bool = False) -> str:
    ...
    if ns:
        try:
            ...
            logger.debug("UPR: NS logo URL via API failed", exc_info=True)
            try:
                from app.utils.sector_logo_urls import ns_logo_url
                ...
                logger.debug("UPR: NS logo URL fallback failed", exc_info=True)
    try:
        from app.utils.sector_logo_urls import github_ns_logo_url

        return github_ns_logo_url(iso3) or ""
    except Exception:
        logger.debug("UPR: GitHub NS logo URL failed for %s", iso3, exc_info=True)
        return ""
...
def spef_icon_srcs(*, inline: bool | None = None) -> dict[str, str]:
    ...
    try:
        ...
        logger.debug("UPR: could not read SPEF icon cache", exc_info=True)

    out: dict[str, str] = {}
    try:
        rows = _load_spef_catalog_rows()
    except Exception:
        logger.debug("UPR: could not load SPEF catalog icons", exc_info=True)
        try:
            db.session.rollback()
        except Exception:
            pass
    ...
    try:
        ...
        logger.debug("UPR: could not store SPEF icon cache", exc_info=True)
    return out
