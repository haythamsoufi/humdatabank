...
@login_required
def reopen_assignment_page(aes_id, page_id):
    """Reopen a single submitted page without unlocking the other pages."""
    from app.services.organization.authorization_service import AuthorizationService
    from app.services.assignments.page_submission_service import (
        is_page_submission_enabled,
        reset_page_status,
    ...
    assignment_entity_status = AssignmentEntityStatus.query.get_or_404(aes_id)
    if not AuthorizationService.can_reopen_assignment_page(assignment_entity_status, current_user):
    ...
    if not is_page_submission_enabled(assignment_entity_status):
        flash(_("Per-page submission is not enabled for this assignment."), "warning")
        return redirect(url_for("assignments.view_assignment", aes_id=aes_id))
    ...
    try:
        ...
        flash(_("Page reopened. Other submitted pages remain locked."), "success")
    except Exception as e:
        request_transaction_rollback()
...
@bp.route("/return_assignment_page/<int:aes_id>/<int:page_id>", methods=["POST"])
@login_required
def return_assignment_page_for_revision(aes_id, page_id):
    """Return a single submitted page for revision without unlocking the others."""
    from app.services.organization.authorization_service import AuthorizationService
    from app.services.assignments.page_submission_service import (
        is_page_submission_enabled,
        mark_page_requires_revision,
    )

    assignment_entity_status = AssignmentEntityStatus.query.get_or_404(aes_id)
    if not AuthorizationService.can_return_assignment_page(assignment_entity_status, current_user):
        flash(_("You do not have permission to return this page for revision."), "danger")
        return redirect(url_for("assignments.view_assignment", aes_id=aes_id))
    ...
    try:
        ...
        flash(_("Page returned for revision. Other submitted pages remain locked."), "success")
    except Exception as e:
        request_transaction_rollback()
        flash(_("Error returning page for revision."), "danger")
        current_app.logger.error(
            "Error returning page %s on assignment %s: %s", page_id, aes_id, e, exc_info=True
        )
    return redirect(url_for("assignments.view_assignment", aes_id=aes_id))
