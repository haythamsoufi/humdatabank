    ...
    try {
      await sentry.SentryFlutter.init(
        (options) {
          ...
          options.release = '${AppConfig.appName}@${AppBuildMetadata.appVersionSemver}'
              '${AppBuildMetadata.buildNumber != null ? "+${AppBuildMetadata.buildNumber}" : ""}';
...
class _MyAppState extends State<MyApp> with WidgetsBindingObserver {
  ...
  Future<void> _refreshSessionOnResume() async {
    ...
    try {
      final jwtService = sl<JwtTokenService>();
  ...
  Widget build(BuildContext context) {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      sl<PerformanceService>().recordFirstFrame();

      Future.delayed(const Duration(seconds: 1), () async {
        final summary = await sl<PerformanceService>().getSummary();
        summary.logSummary();
      });
    });
    ...
    return MultiProvider(
      ...
      child: Consumer2<LanguageProvider, ThemeProvider>(
        builder: (context, languageProvider, themeProvider, child) {
          ...
          sl<PushNotificationService>().navigatorKey = appNavigatorKey;
