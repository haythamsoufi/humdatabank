"""Extended tests for plugins/fdrs/data_quality/fdrs_v1.py.
...
class TestComputeNoAssignment:
    def test_returns_zero_score_when_aes_not_found(self):
        ...
        with patch(
            "plugins.fdrs.data_quality.fdrs_v1.get_assignment_aes",
            return_value=None,
        ):
...
class TestComputeWithAesId:
    def test_uses_provided_aes_id_to_look_up_aes(self):
        ...
        with patch(
            "plugins.fdrs.data_quality.fdrs_v1.AssignmentEntityStatus"
        ) as mock_aes_cls, patch(
            "plugins.fdrs.data_quality.fdrs_v1.load_form_data_by_kpi",
            return_value={},
        ), patch.object(
            ...
            "plugins.fdrs.data_quality.fdrs_v1.validation_question_counts",
...
class TestDocumentsScore:
    def test_no_doc_items_returns_zero(self):
        ...
        with patch(
            "plugins.fdrs.data_quality.fdrs_v1.FormItem.query",
            mock_query,
        ):
    ...
    def test_both_docs_present_returns_full_score(self):
        ...
        with patch(
            ...
            "plugins.fdrs.data_quality.fdrs_v1.SubmittedDocument.query",
            mock_doc_query,
        ):
            score, detail = methodology._documents_score(aes_id=1, template_id=21)
...
class TestTimelinessFallbackPath:
    def test_no_submitted_at_queries_form_data(self):
        ...
        with patch(
            "plugins.fdrs.data_quality.fdrs_v1.FormSection.query"
        ) as mock_sec_q, patch(
            "plugins.fdrs.data_quality.fdrs_v1.FormItem.query"
        ) as mock_item_q, patch(
            "plugins.fdrs.data_quality.fdrs_v1.db"
        ) as mock_db:
...
class TestTrend:
    def test_trend_is_list_of_dicts(self):
        ...
        with patch(
            "app.models.assignments.AssignmentEntityStatus"
        ) as mock_aes_cls, patch(
            "app.models.AssignedForm"
        ), patch.object(
            methodology,
            "compute",
