...
from plugins.upr.formatters import format_compact_chf, planning_years, to_number
...
from plugins.upr.i18n import localized_ns_display_name
    ...
    *,
    ...
    if country_id and years:
        try:
            ...
            logger.exception(
                "UPR: failed to load T22 confirmed PNS funding for host country %s",
                country_id,
            )
    ...
    *,
    ...
    if host_ns_id:
        try:
            ...
            logger.exception("UPR: failed to load T23 PNS funding rows for host NS %s", host_ns_id)
    if host_country_id:
        try:
            for ns_id, value in _load_t22_funding_by_pns(int(host_country_id), period_name, year).items():
                amounts.setdefault(int(ns_id), value)
        except Exception:
            logger.exception("UPR: failed to load T22 PNS funding rows for host country %s", host_country_id)
