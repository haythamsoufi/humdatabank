...
# ---------------------------------------------------------------------------
# power_query_workbook
# ---------------------------------------------------------------------------


class TestPowerQueryWorkbook:
    def test_missing_queries_returns_400(self, logged_in_client, db_session, app):
        with patch("app.routes.admin.shared.AuthorizationService.is_admin", return_value=True), \
             patch("app.routes.admin.shared.AuthorizationService.has_rbac_permission", return_value=True):
    ...
    def test_empty_queries_returns_400(self, logged_in_client, db_session, app):
        with patch("app.routes.admin.shared.AuthorizationService.is_admin", return_value=True), \
             patch("app.routes.admin.shared.AuthorizationService.has_rbac_permission", return_value=True):
            resp = logged_in_client.post(
                "/admin/api-management/power-query-workbook",
    ...
    def test_non_list_queries_returns_400(self, logged_in_client, db_session, app):
        with patch("app.routes.admin.shared.AuthorizationService.is_admin", return_value=True), \
             patch("app.routes.admin.shared.AuthorizationService.has_rbac_permission", return_value=True):
            resp = logged_in_client.post(
                "/admin/api-management/power-query-workbook",
                json={"queries": "not-a-list"},
            )
        assert resp.status_code == 400

    def test_success_returns_xlsx(self, logged_in_client, db_session, app):
        with patch("app.routes.admin.shared.AuthorizationService.is_admin", return_value=True), \
             patch("app.routes.admin.shared.AuthorizationService.has_rbac_permission", return_value=True), \
             patch("app.routes.admin.api_management.build_power_query_workbook", return_value=b"PK fake xlsx"):
            resp = logged_in_client.post(
                "/admin/api-management/power-query-workbook",
                json={"queries": [{"url": "/api/v1/data"}]},
            )
        assert resp.status_code == 200
    ...
    def test_value_error_returns_400(self, logged_in_client, db_session, app):
        with patch("app.routes.admin.shared.AuthorizationService.is_admin", return_value=True), \
             patch("app.routes.admin.shared.AuthorizationService.has_rbac_permission", return_value=True), \
             patch("app.routes.admin.api_management.build_power_query_workbook", side_effect=ValueError("bad")):
            resp = logged_in_client.post(
                "/admin/api-management/power-query-workbook",
                json={"queries": [{"url": "/api/v1/data"}]},
            )
    ...
    def test_exception_returns_500(self, logged_in_client, db_session, app):
        with patch("app.routes.admin.shared.AuthorizationService.is_admin", return_value=True), \
             patch("app.routes.admin.shared.AuthorizationService.has_rbac_permission", return_value=True), \
             patch("app.routes.admin.api_management.build_power_query_workbook", side_effect=RuntimeError("boom")):
            resp = logged_in_client.post(
                "/admin/api-management/power-query-workbook",
                json={"queries": [{"url": "/api/v1/data"}]},
            )
    ...
    def test_custom_filename(self, logged_in_client, db_session, app):
        with patch("app.routes.admin.shared.AuthorizationService.is_admin", return_value=True), \
             patch("app.routes.admin.shared.AuthorizationService.has_rbac_permission", return_value=True), \
             patch("app.routes.admin.api_management.build_power_query_workbook", return_value=b"PK data"):
            resp = logged_in_client.post(
                "/admin/api-management/power-query-workbook",
                json={"queries": [{"url": "/api/v1/data"}], "filename": "my-export"},
            )
        assert resp.status_code == 200
        cd = resp.headers.get("Content-Disposition", "")
        assert "my-export.xlsx" in cd

    def test_filename_already_has_xlsx(self, logged_in_client, db_session, app):
        with patch("app.routes.admin.shared.AuthorizationService.is_admin", return_value=True), \
             patch("app.routes.admin.shared.AuthorizationService.has_rbac_permission", return_value=True), \
             patch("app.routes.admin.api_management.build_power_query_workbook", return_value=b"PK data"):
            resp = logged_in_client.post(
                "/admin/api-management/power-query-workbook",
                json={"queries": [{"url": "/api/v1/data"}], "filename": "my-export.xlsx"},
            )
        assert resp.status_code == 200
