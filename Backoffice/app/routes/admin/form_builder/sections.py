...
def _b64_decode_field(value):
    """Decode a WAF-safe base64-encoded field value sent by the client.

    The JS submit handler base64-encodes ``relevance_condition`` and
    ``name_translations`` so that Azure OWASP CRS WAF rules (e.g. 942100)
    do not misinterpret JSON operators (AND/OR/==) as SQL injection.
    Falls back to the original value when the input is not valid base64,
    preserving backward compatibility for any plain-text requests.
    """
...
def new_template_section(template_id):
    ...
    if form.validate_on_submit():
        try:
            ...
            # and would miss the unprefixed HTML field name) and decode the WAF-safe base64 encoding.
            name_translations_raw_new = _b64_decode_field(data.get('name_translations'))
            if name_translations_raw_new:
                try:
                    name_translations = json.loads(name_translations_raw_new)
...
def edit_template_section(section_id):
    ...
    try:
        ...
        name_translations_str = _b64_decode_field(data.get("name_translations"))
...
def configure_dynamic_section(section_id):
    ...
    try:
        ...
        # Update add_indicator_note_translations (stored in config JSON)
        raw_translations = data.get('add_indicator_note_translations', '{}')
        try:
            translations_dict = json.loads(raw_translations) if raw_translations else {}
        except (ValueError, TypeError):
            translations_dict = {}
        ...
        if translations_dict:
            section.config = {**section.config, 'add_indicator_note_translations': translations_dict}
        else:
            section.config = {k: v for k, v in section.config.items() if k != 'add_indicator_note_translations'}
