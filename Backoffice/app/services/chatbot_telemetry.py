...
class ChatbotTelemetryService:
    ...
    # Class-level flag so _ensure_telemetry_table() only runs once per process.
    # The table is created by Flask-Migrate (add_chatbot_telemetry_table migration).
    # The runtime fallback remains for SQLite dev environments that haven't migrated.
    ...
    def _flush_metrics(self):
        ...
        try:
            # Create table if not exists (safety net; migration should have created it)
            if not ChatbotTelemetryService._table_ensured:
