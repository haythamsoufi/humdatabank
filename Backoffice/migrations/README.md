...
Migrations live in `versions/`. Run from the `Backoffice` directory (set `FLASK_APP=run.py` if needed). Before creating or applying migrations run **`python -m flask db heads`** — **exactly one head** must exist (see **[Developer handbook](../../docs/DEVELOPER-HANDBOOK.md)** → Migration and Data Management).
