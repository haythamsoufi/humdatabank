"""UPR → InDesign IDML and Word-narrative PDF."""
...
from plugins.upr.idml.builder import (
    build_indesign_package,
    zip_indesign_package,
)
from plugins.upr.idml.narrative_pdf import merge_report_pdfs, render_narrative_pdf_bytes
from plugins.upr.idml.narrative_style import folio_label, folio_text, style_narrative_blocks
from plugins.upr.idml.word_reader import load_narrative_paragraphs, load_word_paragraphs
...
def _reject_oversized_zip_member(info: zipfile.ZipInfo) -> None:
    if int(getattr(info, "file_size", 0) or 0) > DOCX_MAX_UNCOMPRESSED_BYTES:
        raise UprError("The Word document is too large to process.")
...
def validate_docx_bytes(data: bytes) -> None:
    if not data:
        raise UprError("Upload a Word document (.docx).")
    if len(data) > DOCX_MAX_BYTES:
        raise UprError("Upload a Word document (.docx) of 20 MB or less.")
...
def validate_pdf_bytes(data: bytes) -> None:
    if not is_pdf_bytes(data):
        raise UprError("Upload a Word document (.docx) or PDF.")
    if len(data) > PDF_MAX_BYTES:
        raise UprError("Upload a Word document (.docx) or PDF of 20 MB or less.")
    import fitz
    ...
    try:
        ...
        if doc.page_count < 1:
            raise UprError("This PDF has no pages.")
        if doc.page_count > PDF_MAX_PAGES:
            raise UprError(f"Upload a PDF of {PDF_MAX_PAGES} pages or fewer.")
    finally:
        doc.close()
