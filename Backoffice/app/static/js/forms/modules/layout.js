...
function initializeFlexibleLayout() {
    ...
    sections.forEach((section, index) => {
        ...
        // Find the collapsible content container within the section
        // The template uses data-collapsible-content with responsive spacing classes
        // (e.g. "space-y-4 sm:space-y-6"), so we cannot rely on a bare .space-y-6 selector.
        const container = section.querySelector('[data-collapsible-content]');
        if (!container) {
            debugLog('layout', `❌ No [data-collapsible-content] container found in section ${section.id}`);
            return;
        }
        debugLog('layout', '✅ Found [data-collapsible-content] container');
...
function applyFlexibleLayoutToSection(container) {
    ...
    // The template uses responsive pairs (space-y-4 sm:space-y-6 for parent sections,
    // space-y-3 sm:space-y-4 for sub-sections) so we must remove all variants explicitly.
    container.classList.remove('space-y-6', 'space-y-4', 'space-y-3');
    ...
    const preservedSubSections = subSections.map(subSection => {
        ...
        const subSectionContainer = clone.querySelector('[data-collapsible-content]');
    ...
    // NOTE: Exclude flex-row wrappers previously generated by this function (marked data-layout-row).
    // Those wrappers still contain clones of indicator elements; re-adding them causes indicators to
    // appear multiple times in the DOM after each subsequent layout pass.
    const preservedExtras = Array.from(container.children).filter(child => {
        ...
        // Skip layout-generated flex-row wrappers (they will be recreated by createFlexibleLayoutContainer)
        if (child.dataset && child.dataset.layoutRow) return false;
                ...
                mutation.addedNodes.forEach(node => {
                    if (node.nodeType === 1 && node.classList.contains('repeat-entry')) {
                        ...
                        if (section) {
                            ...
                            const container = section.querySelector('[data-collapsible-content]');
                            if (container) {
                                debugLog('layout', 'Found [data-collapsible-content] container, refreshing layout');
                                refreshSectionLayout(container);
                            } else {
                                debugLog('layout', '❌ No [data-collapsible-content] container found in repeat section');
                            }
                        } else {
...
function applyLayoutToSubSection(subSectionContainer) {
    ...
    // Reduce vertical spacing inside sub-sections.
    // Sub-section containers use responsive pairs (space-y-3 sm:space-y-4) so remove all variants.
    subSectionContainer.classList.remove('space-y-4', 'space-y-3', 'space-y-2');
    subSectionContainer.classList.add('space-y-2');
...
function createFlexibleLayoutContainer(fieldGroups) {
    ...
    fieldGroups.forEach((group, groupIndex) => {
        ...
        // Mark as a layout-generated wrapper so subsequent layout passes can identify and exclude it
        // from preservedExtras (prevents ghost indicator copies accumulating on each re-layout).
        rowDiv.dataset.layoutRow = 'true';
...
export function applyLayoutToSection(sectionContainer) {
    ...
    const fieldsContainer = sectionContainer.querySelector('[data-collapsible-content]');
    if (!fieldsContainer) {
        return;
    }
