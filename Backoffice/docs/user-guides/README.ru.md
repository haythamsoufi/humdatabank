# Руководства пользователя

Пошаговые рабочие процессы и обучающие материалы по использованию системы бэк-офиса Humanitarian Databank.

## Доступные руководства

### Админ

- [Add a user](admin/add-user.md)
- [Manage users](admin/manage-users.md)
- [User roles](admin/user-roles.md)
- [Role recipes](admin/role-recipes.md)
- [Create a form template](admin/create-template.md)
- [Edit a template (Form Builder)](admin/edit-template.md)
- [Form Builder (advanced)](admin/form-builder-advanced.md)
- [Create and manage assignments](admin/manage-assignments.md)
- [Create assignment](admin/create-assignment.md)
- [Assignment lifecycle](admin/assignment-lifecycle.md)
- [Run a reporting cycle](admin/run-a-reporting-cycle.md)
- [Review and approve submissions](admin/review-approve-submissions.md)
- [Public URL submissions](admin/public-url-submissions.md)
- [Export and download data](admin/export-download-data.md)
- [Exports: how to interpret files](admin/exports-how-to-interpret.md)
- [Indicator Bank](admin/indicator-bank.md)
- [AI Document Library and embeddings](admin/ai-document-library-and-embeddings.md)
- [AI system: security and privacy](admin/ai-system-security-and-privacy.md)
- [Supporting documents (Admin)](admin/supporting-documents.md)
- [Troubleshooting access (Admin)](admin/troubleshooting-access.md)
- [Troubleshooting templates and assignments (Admin)](admin/troubleshooting-templates-and-assignments.md)
- [Notifications and communications (Admin)](admin/notifications-and-communications.md)

### Центр внимания

- [View your assignments](focal-point/view-assignments.md)
- [Fill and submit a form](focal-point/submit-data.md)
- [Troubleshooting](focal-point/troubleshooting.md)
- [Quality Index](focal-point/data-quality-index.md)

### Обычный

- [Submission Status](common/submission-statuses-and-permissions.md)
- [AI Use Policy](common/ai-use-policy.md)
- [AI Chatbot](common/ai-chatbot.md) – Как работает чат-бот, уровни доступа, конфиденциальность документов и доступные инструменты
- [Data governance: how the system supports it](common/data-governance.md)
- [Getting help](common/getting-help.md)

### Начало

- [Start here](../getting-started/start-here.md)
- [How the platform works](../getting-started/how-it-works.md)
- [Glossary](../getting-started/glossary.md)
- [Account settings](../getting-started/account-settings.md)

### Отчётность по данным

- [Data Guidance, Unified Planning and Reporting (UPR)](../data-reporting/data-guidance-upr.md)
- [Data Guidance, FDRS](../data-reporting/data-guidance-fdrs.md)
- [Supporting documents (Focal point)](../data-reporting/supporting-documents.md)
- [Data handling and privacy](../data-reporting/data-handling-and-privacy.md)

## Язык

Документация отображается на выбранном языке в **Настройках аккаунта**. Поддерживаемые языки гидов включают **английский**, **французский**, **испанский**, **арабский** и **русский**. Если страница ещё не переведена, показывается английская версия.
