...
from .workflow_docs_service import WorkflowDocsService
...
def _is_redundant_bookmark_anchor(tag) -> bool:
    """True for empty <a id="..."></a> elements used only as link targets."""
    return (
        getattr(tag, "name", None) == "a"
        and not (tag.get("href") or "").strip()
        and not tag.get_text(strip=True)
    )


def _deduplicate_html_ids(html: str) -> str:
    """
    Remove redundant empty anchor targets and resolve duplicate element IDs.

    Docs sometimes include manual ``<a id="section"></a>`` markers that duplicate
    heading IDs generated by the Markdown TOC extension. WeasyPrint warns on
    duplicate anchors; browsers also behave unpredictably with duplicate IDs.
    """
    soup = BeautifulSoup(html, "html.parser")
    seen: Dict[str, object] = {}

    for el in list(soup.find_all(id=True)):
        id_val = (el.get("id") or "").strip()
        if not id_val:
            continue
        ...
        prev = seen[id_val]
        if _is_redundant_bookmark_anchor(el):
            el.decompose()
            continue
        if _is_redundant_bookmark_anchor(prev):
            prev.decompose()
            seen[id_val] = el
            continue
        ...
        existing = {tag.get("id") for tag in soup.find_all(id=True) if tag.get("id")}
