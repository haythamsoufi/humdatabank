...
async function loadDataQualityScore(templateId, period, entityType, entityId) {
    ...
    if (!templateId || !period || !entityType || entityId == null || entityId === '') {
        if (panelContent) {
            panelContent.innerHTML = '<p class="text-gray-500 text-sm py-6 text-center">' + window.t('No reporting period available for this template.') + '</p>';
        }
        return;
    }
    ...
    try {
        ...
        if (contentType.includes('application/json')) {
            ...
            if (panelContent) {
                panelContent.innerHTML = '<p class="text-red-600 text-sm">' + window.t('Failed to load data quality score. Please refresh or contact support.') + '</p>';
            }
            return;
        }
        if (!resp.ok) {
            if (panelContent) panelContent.innerHTML = `<p class="text-red-600 text-sm">${sanitizeTextContent(data.error || window.t('Failed to load score'))}</p>`;
            return;
        }
    ...
    document.addEventListener('submit', function(event) {
        ...
        if (form.classList.contains('delete-self-report-form')) {
            ...
            if (window.showDangerConfirmation) {
                window.showDangerConfirmation(msg, () => { form.dataset.confirmed = 'true'; form.requestSubmit ? form.requestSubmit() : form.submit(); }, _dsrRestore, window.t('Delete'), window.t('Cancel'), window.t('Confirm Delete'));
            } else if (window.showConfirmation) {
                window.showConfirmation(msg, () => { form.dataset.confirmed = 'true'; form.requestSubmit ? form.requestSubmit() : form.submit(); }, _dsrRestore, window.t('Delete'), window.t('Cancel'), window.t('Confirm Delete'));
            } else {
                _dsrRestore();
        ...
        if (form.classList.contains('approve-assignment-form')) {
            ...
            if (window.showConfirmation) {
                window.showConfirmation(msg, () => { form.dataset.confirmed = 'true'; form.requestSubmit ? form.requestSubmit() : form.submit(); }, _aaRestore, window.t('Approve'), window.t('Cancel'), window.t('Approve Assignment?'));
            } else {
                _aaRestore();
                console.warn('Confirmation dialog not available:', msg);
                return false;
        ...
        if (form.classList.contains('reopen-assignment-form')) {
            ...
            if (window.showConfirmation) {
                window.showConfirmation(msg, () => { form.dataset.confirmed = 'true'; form.requestSubmit ? form.requestSubmit() : form.submit(); }, _raRestore, window.t('Reopen'), window.t('Cancel'), window.t('Reopen Assignment?'));
            } else {
                _raRestore();
                console.warn('Confirmation dialog not available:', msg);
                return false;
