...
class AuthProvider with ChangeNotifier {
  ...
  /// Set when a session is definitively invalidated by the server (e.g. token
  /// blacklisted, refresh rejected, 401 after silent-refresh).  Cleared on
  /// successful login or explicit logout.  Not set for transient failures.
  String? _sessionInvalidationReason;
  ...
  /// Non-null when the most-recent session was definitively invalidated by the
  ...
  Future<bool> checkAuthStatus({bool forceRevalidate = false}) async {
    ...
    try {
      ...
      _error = e.toString();
      _sessionInvalidationReason = e.toString();
      DebugLogger.logWarn(
