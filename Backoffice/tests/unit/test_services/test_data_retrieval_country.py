...
class TestCheckCountryAccess:
    def test_unrestricted_user_returns_true(self, app, db_session):
        with app.app_context():
            from app.services.data_retrieval_country import check_country_access
            country = create_test_country(db_session)
            with patch("app.services.data_retrieval_country.user_allowed_country_ids", return_value=None):
    ...
    def test_country_in_allowed_set_returns_true(self, app, db_session):
        with app.app_context():
            from app.services.data_retrieval_country import check_country_access
            country = create_test_country(db_session)
            with patch("app.services.data_retrieval_country.user_allowed_country_ids",
    ...
    def test_country_not_in_allowed_set_checks_entity_service(self, app, db_session):
        with app.app_context():
            ...
            with patch("app.services.data_retrieval_country.user_allowed_country_ids",
                       return_value={99999}), \
                 patch("app.services.data_retrieval_country.current_user", mock_user), \
...
class TestResolveCountry:
    ...
    def test_exception_returns_none(self, app):
        with app.app_context():
            from app.services.data_retrieval_country import resolve_country
            with patch("app.services.data_retrieval_country.db") as mock_db:
...
class TestGetCountryInfo:
    def test_country_not_found_returns_error(self, app, db_session):
        with app.app_context():
            from app.services.data_retrieval_country import get_country_info
    ...
    def test_access_denied_returns_error(self, app, db_session):
        with app.app_context():
            from app.services.data_retrieval_country import get_country_info
            country = create_test_country(db_session)
            with patch("app.services.data_retrieval_country.check_country_access", return_value=False):
    ...
    def test_found_with_no_assignments(self, app, db_session):
        with app.app_context():
            from app.services.data_retrieval_country import get_country_info
            country = create_test_country(db_session)
            with patch("app.services.data_retrieval_country.check_country_access", return_value=True):
                result = get_country_info(country.id)
...
class TestGetAssignmentsForCountry:
    def test_access_denied_returns_empty(self, app, db_session):
        with app.app_context():
            from app.services.data_retrieval_country import get_assignments_for_country
            country = create_test_country(db_session)
            with patch("app.services.data_retrieval_country.check_country_access", return_value=False):
                mock_user = MagicMock()
                mock_user.id = 1
                with patch("app.services.data_retrieval_country.current_user", mock_user):
    ...
    def test_exception_returns_empty(self, app, db_session):
        with app.app_context():
            ...
            country = create_test_country(db_session)
            with patch("app.services.data_retrieval_country.check_country_access", return_value=True), \
                 patch("app.services.data_retrieval_country.db") as mock_db:
...
class TestGetUserCountries:
    def test_unrestricted_returns_all_countries(self, app, db_session):
        with app.app_context():
            from app.services.data_retrieval_country import get_user_countries
...
class TestGetUserCountryIds:
    def test_unrestricted_returns_all_ids(self, app, db_session):
        with app.app_context():
            from app.services.data_retrieval_country import get_user_country_ids
            c1 = create_test_country(db_session)
            c2 = create_test_country(db_session)
            with patch("app.services.data_retrieval_country.user_allowed_country_ids", return_value=None):
