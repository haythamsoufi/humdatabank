...
def _is_matrix_share_rows(d: dict) -> bool:
    """Cross-country form matrix rows with share_breakdown (any row count)."""
    if not d.get("matrix_share_rows"):
        return False
    rows = d.get("rows")
    if not isinstance(rows, list) or not rows:
        return False
    sample = next((r for r in rows if isinstance(r, dict)), None)
    return bool(sample and sample.get("share_breakdown"))
...
def _is_country_rows(d: dict) -> bool:
    ...
    if not sample:
        return False
    return bool(sample.get("iso3") or sample.get("country_iso3")) and (
        "value" in sample or "total" in sample
    )
...
def _matrix_share_column_defs(d: dict) -> List[Dict[str, Any]]:
    ...
    columns: List[Dict[str, Any]] = [
        ...
        {"key": "region", "label": _("Operational region"), "sortable": True, "type": "text"},
        {"key": "period", "label": _("Period"), "sortable": True, "type": "text"},
        {"key": "total", "label": _("Total funding"), "sortable": True, "type": "number"},
    ]
    for label in d.get("matrix_share_rows") or []:
        slug = slugify_matrix_row_label(str(label))
        short = str(label).replace(" Government", " gov.").strip()
        columns.append({
            "key": f"{slug}_pct",
            "label": f"{short} %",
            "sortable": True,
    ...
    columns.append({
        "key": "matching_sources",
        "label": _("Matching source"),
        "sortable": True,
        "type": "text",
    })
    return columns
...
def _build_matrix_share_table(d: dict, query: str = "") -> Optional[Dict[str, Any]]:
    rows = d.get("rows")
    if not isinstance(rows, list) or not rows:
        return None

    field = str(d.get("field_label_resolved") or "Form matrix").strip()
    template = str(d.get("template_name") or "").strip()
    min_pct = d.get("min_share_pct")
    if isinstance(min_pct, (int, float)):
        title = f"{field} — ≥{min_pct:g}% share"
    else:
        title = f"{field} — share analysis"
    if template:
        title = f"{template}: {title}"

    columns = _matrix_share_column_defs(d)
    table_rows: List[Dict[str, Any]] = []
    for r in rows:
        if not isinstance(r, dict):
            continue
        matching = r.get("matching_sources") or []
        row: Dict[str, Any] = {
            "country_name": str(r.get("country_name") or "").strip(),
            "iso3": str(r.get("iso3") or "").strip().upper(),
            "region": str(r.get("region") or "").strip(),
            "period": str(r.get("period_used") or "").strip(),
            "total": _parse_numeric(r.get("share_denominator") or r.get("total")),
            "max_share_pct": _parse_numeric(r.get("max_share_pct")),
            "matching_sources": ", ".join(str(m) for m in matching) if matching else "—",
        }
        for col in columns:
            key = col.get("key")
            if key.endswith("_pct") and key not in row:
                row[key] = _parse_numeric(r.get(key))
    ...
    return {
        ...
        "sort_by": "max_share_pct",
        "sort_order": "desc",
        "table_kind": "matrix_share",
        "_score": len(table_rows) + 200,
    }
...
def _build_country_table(d: dict, query: str = "") -> Optional[Dict[str, Any]]:
    ...
    for r in rows:
        ...
        row: Dict[str, Any] = {
            ...
            "iso3": str(r.get("iso3") or "").strip().upper(),
            "region": str(r.get("region") or "").strip(),
            "value": _parse_numeric(r.get("value") if r.get("value") is not None else r.get("total")),
        }
        if has_period:
...
_REGISTRY: List[Tuple[Callable, Callable, str, int]] = [
    ...
    (_is_matrix_share_rows,   _build_matrix_share_table,     "table_payload", 12),
...
def _apply_enrichment(
    ...
    if table_payload.get("table_kind") in {"unified_plans_focus", "indicator_search", "matrix_share"}:
        if table_payload.get("table_kind") in {"indicator_search", "matrix_share"}:
            return table_payload
        combined = f"{query or ''}\n{answer_text or ''}"
