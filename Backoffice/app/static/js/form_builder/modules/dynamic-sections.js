...
export const DynamicSections = {
    ...
    updateIndicatorCountDisplay(sectionId) {
        ...
        // WAF: wrap body in { payload: b64 } — filter values (sector/subsector names etc.) can
        // trigger OWASP CRS rules. get_request_data() unwraps transparently on the server.
        const _dsFilterB64 = btoa(unescape(encodeURIComponent(JSON.stringify({ filters: activeFilters }))));
        CsrfHandler.safeFetch('/admin/api/indicator-count', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ payload: _dsFilterB64 })
        })
...
// ── Indicator Note Translation Modal (shared across all dynamic sections) ─────
(function setupIndicatorNoteTranslations() {
    // Defer until DOM ready; the modal lives in form_builder.html, always present.
    document.addEventListener('DOMContentLoaded', function () {
        const modal = document.getElementById('indicator-note-translation-modal');
        if (!modal) return;

        const saveBtn = document.getElementById('save-indicator-note-translations-btn');
        const clearBtn = document.getElementById('clear-indicator-note-btn');
        const autoBtn = document.getElementById('auto-translate-indicator-note-btn');
        const CSS_PREFIX = 'indicator-note';

        let activeSectionId = null;

        // Open: delegated — any .indicator-note-translation-btn on the page
        document.addEventListener('click', function (e) {
            const btn = e.target.closest('.indicator-note-translation-btn');
            if (!btn) return;

            activeSectionId = btn.dataset.sectionId;
            if (!activeSectionId) return;

            // Read existing translations from the section-specific hidden input
            let translations = {};
            const hiddenInput = document.getElementById(`add_indicator_note_translations_${activeSectionId}`);
            if (hiddenInput && hiddenInput.value) {
                try { translations = JSON.parse(hiddenInput.value); } catch (_) {}
            }

            if (window.TranslationModalUtils) {
                window.TranslationModalUtils.populateFields(CSS_PREFIX, translations);
            }
        ...
        if (saveBtn) {
            saveBtn.addEventListener('click', function () {
                if (!activeSectionId) { modal.classList.add('hidden'); return; }

                const collected = (window.TranslationModalUtils && typeof window.TranslationModalUtils.collectFields === 'function')
                    ? window.TranslationModalUtils.collectFields(CSS_PREFIX)
                    : {};

                // Filter out blank values
                const filtered = {};
                Object.entries(collected).forEach(([k, v]) => { if (v && String(v).trim()) filtered[k] = v; });

                const hiddenInput = document.getElementById(`add_indicator_note_translations_${activeSectionId}`);
                if (hiddenInput) hiddenInput.value = JSON.stringify(filtered);

                modal.classList.add('hidden');

                // Flash the trigger button as confirmation
                const triggerBtn = document.querySelector(`.indicator-note-translation-btn[data-section-id="${activeSectionId}"]`);
                if (triggerBtn) {
                    const orig = Array.from(triggerBtn.childNodes).map(n => n.cloneNode(true));
                    triggerBtn.replaceChildren();
                    const icon = document.createElement('i');
                    icon.className = 'fas fa-check w-4 h-4 mr-1';
                    triggerBtn.append(icon, document.createTextNode('Saved'));
                    triggerBtn.classList.add('text-green-600');
                    setTimeout(() => {
                        triggerBtn.replaceChildren(...orig.map(n => n.cloneNode(true)));
                        triggerBtn.classList.remove('text-green-600');
                    }, 2000);
                }
        ...
        if (clearBtn) {
            clearBtn.addEventListener('click', function () {
                if (window.TranslationModalUtils) {
                    window.TranslationModalUtils.clearFields(CSS_PREFIX);
                }
            });
        }
        ...
        if (autoBtn) {
            autoBtn.addEventListener('click', function () {
                ...
                const noteTextarea = document.getElementById(`add_indicator_note_${activeSectionId}`);
                const englishText = noteTextarea ? noteTextarea.value.trim() : '';
                ...
                const restoreBtn = () => { autoBtn.replaceChildren(...originalNodes.map(n => n.cloneNode(true))); };

                autoBtn.replaceChildren();
                const spinner = document.createElement('i');
                spinner.className = 'fas fa-spinner fa-spin w-4 h-4 mr-2';
                autoBtn.append(spinner, document.createTextNode('Translating...'));
                ...
                (window.AutoTranslateService && typeof window.AutoTranslateService.translate === 'function'
                    ...
                    if (window.TranslationModalUtils && typeof window.TranslationModalUtils.showAutoTranslateError === 'function') {
                        window.TranslationModalUtils.showAutoTranslateError(autoBtn, 'Auto Translate', error.message, { originalNodes });
                    } else {
                        restoreBtn();
                        autoBtn.disabled = false;
                    }
