...
function attachDiscussionModal() {
  if (!window.TranslationModal || !document.getElementById('discussion-translation-modal')) return;
  window.TranslationModal.attach({
    openButtonId: 'discussion-translations-btn',
    modalId: 'discussion-translation-modal',
    cssPrefix: 'discussion',
    resolveEnglishText: () => (document.getElementById('item-discussion-title')?.value || ''),
    onSaveHiddenFields: (collectedByTab) => {
      const labelTranslationsInput = document.getElementById('item-modal-shared-label-translations');
      const descriptionTranslationsInput = document.getElementById('item-modal-shared-description-translations');
      if (labelTranslationsInput) labelTranslationsInput.value = JSON.stringify(collectedByTab.labels || {});
    ...
    onModalOpen: () => {
      ...
      if (window.TranslationModalUtils) {
        window.TranslationModalUtils.populateFields('discussion', labelTranslations, '', 'labels');
        window.TranslationModalUtils.populateFields('discussion', descriptionTranslations, '', 'descriptions');
      }
    }
  });
}
