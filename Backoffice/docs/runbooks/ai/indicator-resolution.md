...
- **Vector-only (no LLM)**: Simple and fast; for ambiguous queries returning top‑1 by score is usually enough; LLM adds clarity when needed.
...
1. **Indicator Bank embeddings** — Table: `indicator_bank_embeddings` (indicator_bank_id, embedding vector, text_embedded, model, dimensions). Text per indicator: `name` + `definition` + `unit` (and optional translation snippets). Same embedding model/dimensions as RAG (`AI_EMBEDDING_*`).
2. **Resolution service** (`indicator_resolution_service.py`) — `resolve()`, `resolve_with_llm()`; sync when the bank changes.
3. **Config** — `AI_INDICATOR_RESOLUTION_METHOD`: `"vector"` | `"vector_then_llm"` | `"keyword"`; `AI_INDICATOR_LLM_DISAMBIGUATE`; `AI_INDICATOR_TOP_K`.
4. **Integration** — In databank-facing tools (`get_value_breakdown`, `get_indicator_values_for_all_countries`, …), phrase identifiers go through resolver when configured.
