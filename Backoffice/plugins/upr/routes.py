"""HTTP routes for UPR — assignment embed, PNG/PDF/IDML export, admin bulk."""

from __future__ import annotations

from html import escape

from flask import Response, current_app, g, has_request_context, redirect, render_template, request, send_from_directory, url_for
from flask_babel import force_locale, gettext as _
from flask_login import current_user, login_required
from werkzeug.exceptions import HTTPException, NotFound

from plugins.upr import bp, _PLUGIN_DIR
from plugins.upr.catalog import dashboards_for_kind, kind_for_template
...
from plugins.upr.render import render_dashboard_html, render_dashboards_html, render_report_html
from plugins.upr.audience import resolve_narrative_audience
from plugins.upr.idml import read_narrative_upload
from plugins.upr.assignment_job import (
...
from plugins.upr.bulk_job import (
...
from app.routes.admin.shared import admin_required, permission_required, system_manager_required
...
@bp.app_template_global("upr_static_url")
def upr_static_url(filename: str) -> str:
...
@bp.app_template_global("upr_asset_v")
def upr_asset_v() -> str:
    return _plugin_asset_version("css/upr.css")
...
def _upr_format_label(export_format: str) -> str:
    key = (export_format or "").strip().lower()
    return _UPR_VISUALS_FORMAT_LABELS.get(key, key.upper() or "export")


def _upr_generation_description(export_format: str, *, narrative: bool = False) -> str:
    fmt = _upr_format_label(export_format)
    if narrative:
        return f"Generated UPR ({fmt} with narrative)"
    return f"Generated UPR ({fmt})"


def _log_upr_generation(
    *,
    export_format: str,
    ...
    """Write an audit-trail row for a newly queued UPR export."""
    try:
        from app.services.organization.entity_service import EntityService
        from app.services.platform.user_analytics_service import log_user_activity

        description = _upr_generation_description(export_format, narrative=narrative)
        ...
        if aes_id:
            ...
            try:
                ...
                current_app.logger.debug("UPR audit entity context skipped: %s", exc)
        ...
        current_app.logger.debug("UPR audit log skipped: %s", exc)


def _render_rate_key() -> str:
    user_id = getattr(current_user, "id", None)
    return f"upr_render_{user_id or 'anon'}"
...
def _aes_or_404(aes_id: int) -> AssignmentEntityStatus:
    ...
    if not assignment_supports_visuals(aes):
        raise UprError("UPR dashboards are only available for Unified Plan and Report assignments.")
    return aes
...
@bp.route("/admin/plugins/upr/settings", methods=["GET"])
@login_required
...
@bp.route("/upr/fonts.css", methods=["GET"])
@login_required
def fonts_css():
    """@font-face + Tajawal inherit rules — same stacks WeasyPrint uses."""
    from plugins.upr.typography import browser_stylesheet, export_style_token
...
def assignment_visuals_progress(aes_id: int):
    try:
        ...
        return handle_json_view_exception(
            exc, GENERIC_ERROR_MESSAGE, log_message=f"UPR progress failed for aes {aes_id}"
        )
...
def assignment_payload(aes_id: int):
    try:
        ...
        return handle_json_view_exception(
            exc, GENERIC_ERROR_MESSAGE, log_message=f"UPR payload failed for aes {aes_id}"
        )
...
def assignment_report(aes_id: int):
    try:
        ...
        return handle_json_view_exception(
            exc, GENERIC_ERROR_MESSAGE, log_message=f"UPR report failed for aes {aes_id}"
        )
...
def _export_wait_response(
    ...
    *,
    ...
    status_url = url_for(
        "upr.assignment_narrative_status", aes_id=aes_id, job_id=job_id
    )
    if not file_url:
        file_params: dict[str, str | int] = {"aes_id": aes_id, "job_id": job_id}
        if not download:
            file_params["inline"] = 1
        file_url = url_for("upr.assignment_narrative_file", **file_params)
    script_url = url_for("upr.static_file", filename="js/upr-export-wait.js")
    html = (
...
def assignment_png(aes_id: int, dashboard_id: str):
    try:
        ...
        return handle_json_view_exception(
            exc, GENERIC_ERROR_MESSAGE, log_message=f"UPR PNG failed for aes {aes_id}"
        )
    ...
    try:
        ...
        if _wants_download() or _wants_raw_pdf() or _prefers_native_pdf_viewer():
            ...
            return _export_wait_response(
                ...
                file_url=url_for("upr.assignment_pdf", **file_params),
            )
        ...
        with force_locale(lang):
            ...
            return _pdf_viewer_response(
                ...
                download_url=url_for("upr.assignment_pdf", **download_params),
                script_url=url_for(
                    "upr.static_file", filename="js/upr-pdf-viewer.js"
                ),
                lang=lang,
            )
    except UprError as exc:
        ...
        return handle_json_view_exception(
            exc, GENERIC_ERROR_MESSAGE, log_message=f"UPR PDF failed for aes {aes_id}"
        )
...
def assignment_idml(aes_id: int):
    try:
        ...
        return handle_json_view_exception(
            exc, GENERIC_ERROR_MESSAGE, log_message=f"UPR IDML failed for aes {aes_id}"
        )
...
def assignment_narrative(aes_id: int):
    try:
        ...
        return handle_json_view_exception(
            exc, GENERIC_ERROR_MESSAGE, log_message=f"UPR narrative export failed for aes {aes_id}"
        )
...
def assignment_narrative_status(aes_id: int):
    try:
        ...
        return handle_json_view_exception(
            exc, GENERIC_ERROR_MESSAGE, log_message=f"UPR narrative status failed for aes {aes_id}"
        )
...
def assignment_narrative_file(aes_id: int, job_id: str):
    try:
        ...
        return handle_json_view_exception(
            exc, GENERIC_ERROR_MESSAGE, log_message=f"UPR narrative download failed for aes {aes_id}"
        )
...
@bp.route("/admin/data-exploration/upr/manage", methods=["GET"])
@permission_required("admin.data_explore.upr")
@system_manager_required
def manage_page():
    return redirect(url_for("data_exploration.explore_data", tab="upr"))


@bp.route("/admin/data-exploration/upr/assignments", methods=["GET"])
@permission_required("admin.data_explore.upr")
@system_manager_required
def assignments():
...
@bp.route("/admin/data-exploration/upr/countries", methods=["GET"])
@permission_required("admin.data_explore.upr")
@system_manager_required
def countries():
    try:
...
@bp.route("/admin/data-exploration/upr/generate", methods=["POST"])
@permission_required("admin.data_explore.upr")
@system_manager_required
def generate():
    ...
    try:
        ...
        g.audit_activity_description = _upr_generation_description(
        ...
        return handle_json_view_exception(exc, GENERIC_ERROR_MESSAGE, log_message="UPR generate failed")


@bp.route("/admin/data-exploration/upr/status", methods=["GET"])
@permission_required("admin.data_explore.upr")
@system_manager_required
def status():
...
@bp.route("/admin/data-exploration/upr/cancel", methods=["POST"])
@permission_required("admin.data_explore.upr")
@system_manager_required
def cancel():
    payload = request.get_json(silent=True) or {}
...
@bp.route("/admin/data-exploration/upr/download/<job_id>", methods=["GET"])
@permission_required("admin.data_explore.upr")
@system_manager_required
def download(job_id: str):
    try:
        ...
        return handle_json_view_exception(exc, GENERIC_ERROR_MESSAGE, log_message="UPR download failed")
