# ========== AI Data Retrieval: Form ==========
"""
AI chatbot form-data retrieval tools (indicator values, breakdowns, bulk cross-country queries).

Values are read from FormData joined with AssignmentEntityStatus and FormItem/IndicatorBank.
Uses shared helpers from data_retrieval_form_helpers and data_retrieval_shared.
"""
...
from app.models import (
    Country, FormTemplate, FormSection, IndicatorBank,
...
from app.services.data_retrieval.form_helpers import (
    ...
    apply_bulk_rbac,
    submission_recency_ts,
    pick_most_recent_per_country,
    resolve_template_by_identifier,
    resolve_form_items_by_label,
    apply_matrix_share_analysis,
    load_submission_values_by_kpi_code,
    enrich_matrix_share_display_rows,
    infer_matrix_share_denominator_kpi_code,
)

logger = logging.getLogger(__name__)


def numeric_from_formdata_value(value: Any, disagg_data: Any) -> Optional[float]:
    """Public alias for extract_numeric_from_formdata (tests / tooling)."""
    return extract_numeric_from_formdata(value, disagg_data)
    ...
    try:
        ...
        for e in by_submission.values():
            if not bool(e.get("has_numeric")):
                ...
                parsed, _year_src, _year_extra = _period_year(e)
    ...
    try:
        ...
        if isinstance(indicator_identifier, int) or (isinstance(indicator_identifier, str) and indicator_identifier.isdigit()):
            ...
            if indicator is None and ident:
                ...
                template = resolve_template_by_identifier(ident)
                if template:
                    return {
...
def get_indicator_values_for_all_countries(
    ...
    try:
        ...
        def _recency_ts(e: dict) -> _dt:
            return submission_recency_ts(e)
        ...
        def _numeric_from_row(value: Any, disagg_data: Any) -> float | None:
            num = extract_numeric_from_formdata(value, disagg_data)
...
def get_assignment_indicator_values(
    ...
    try:
        ...
        template = resolve_template_by_identifier(template_identifier)
        if not template:
        ...
        for fd, fi, ind in fd_rows:
            ...
            num = numeric_from_formdata_value(val, disagg)
            if num is not None:
                val = num
...
def get_form_field_values_for_all_countries(
    *,
    field_label_or_name: str,
    template_identifier: Optional[Union[int, str]] = None,
    assignment_period: Optional[str] = None,
    ...
    Prefer template_identifier to disambiguate fields that appear in multiple templates.
    Optional matrix_share_rows + min_share_pct enable share/threshold filtering (e.g. income
    sources where home government >= 75% of total) without query-specific tooling.
    """
    def _progress(msg: str) -> None:
        if on_progress:
            with suppress(Exception):
    ...
    try:
        search_term = (field_label_or_name or "").strip()
        if not search_term:
            return service_error("Field label or name is required", rows=[], count=0)

        assignment_period = ((assignment_period or "").strip() or None)
        matrix_row_filter = ((matrix_row_filter or "").strip() or None)
        ...
        if template_identifier is not None:
            template = resolve_template_by_identifier(template_identifier)
            if not template:
                return service_error("Template not found", rows=[], count=0)
            template_id = int(template.id)

        items = resolve_form_items_by_label(search_term, template_id=template_id)
        if not items:
            return {
                "success": False,
                "error": f'No form field found matching "{field_label_or_name}".',
        ...
        matched_items = [
            {
                "id": int(i.id),
                "label": i.label,
                "section": i.form_section.name if i.form_section else None,
        ...
        q = apply_bulk_rbac(q, can_see_ifrc, allowed_country_ids, join_form_item=True)
        records = q.all()
        if not records:
            return {
                "success": True,
                "field_label_resolved": field_label_resolved,
                "section_label_resolved": section_label_resolved,
                ...
                "rows": [],
                "count": 0,
                "note": "No form field data found for accessible countries.",
            }

        by_country_submissions: Dict[int, List[dict]] = {}
        template_ids_seen: set = set()
        for r in records:
            cid = int(r.country_id)
            sid = int(r.submission_id)
            breakdown = breakdown_from_disagg(r.disagg_data, matrix_row_filter)
            total = extract_numeric_from_formdata(
                r.value,
                r.disagg_data,
                matrix_row_filter=matrix_row_filter,
            )
            ...
            entry = {
                ...
                "total": total_val,
                "breakdown": breakdown,
                "has_data": bool(total_val or breakdown),
            }
            if entry.get("template_id"):
                template_ids_seen.add(int(entry["template_id"]))
            by_country_submissions.setdefault(cid, []).append(entry)

        picked = pick_most_recent_per_country(by_country_submissions)

        country_ids = sorted(picked.keys())
        countries = Country.query.filter(Country.id.in_(country_ids)).all() if country_ids else []
        country_map = {int(c.id): c for c in countries}

        template_map: Dict[int, FormTemplate] = {}
        if template_ids_seen:
            for t in FormTemplate.query.filter(FormTemplate.id.in_(sorted(template_ids_seen))).all():
                template_map[int(t.id)] = t

        _progress("Building results…")
        rows_out: List[Dict[str, Any]] = []
        for cid, entry in picked.items():
            c = country_map.get(int(cid))
            status = (entry.get("status") or "").strip()
            tmpl_id = entry.get("template_id")
            tmpl = template_map.get(int(tmpl_id)) if tmpl_id else template
            row = {
                "country_id": int(cid),
                "country_name": c.name if c else str(cid),
                "iso3": (getattr(c, "iso3", None) or "") if c else "",
                ...
                "period_used": entry.get("period_name"),
                "data_status": "submitted" if status in ("submitted", "approved") else "saved",
                "assignment_name": tmpl.name if tmpl else None,
        ...
        if min_total is not None:
            try:
                threshold = float(min_total)
                rows_out = [r for r in rows_out if float(r.get("total") or 0) >= threshold]
            except (TypeError, ValueError):
                pass

        share_rows = [str(r).strip() for r in (matrix_share_rows or []) if str(r).strip()]
        share_meta: Dict[str, Any] = {}
        countries_with_data = len(rows_out)
        if share_rows:
            resolved_kpi = infer_matrix_share_denominator_kpi_code(
                field_label=field_label_resolved or search_term,
                template_name=template.name if template else None,
                template_identifier=template_identifier,
                matrix_share_rows=share_rows,
                explicit_kpi_code=denominator_kpi_code,
            )
            denom_template_id = template_id
            if denom_template_id is None and template_ids_seen:
                denom_template_id = next(iter(template_ids_seen))
            denominator_by_submission: Dict[int, float] = {}
            if resolved_kpi and denom_template_id is not None:
                aes_ids = sorted({
                    int(r["submission_id"])
                    for r in rows_out
                    if r.get("submission_id") is not None
                })
                denominator_by_submission = load_submission_values_by_kpi_code(
                    aes_ids,
                    int(denom_template_id),
                    resolved_kpi,
                )
            rows_out = apply_matrix_share_analysis(
                rows_out,
                matrix_share_rows=share_rows,
                matrix_share_column=((matrix_share_column or "").strip() or None),
                min_share_pct=min_share_pct,
                share_match=share_match,
                share_threshold_rows=share_threshold_rows,
                denominator_by_submission=denominator_by_submission,
            )
            rows_out = enrich_matrix_share_display_rows(
                rows_out,
                share_rows,
                min_share_pct=min_share_pct,
            )
            share_meta = {
                ...
                "min_share_pct": min_share_pct,
                "share_match": share_match,
                "denominator_kpi_code": resolved_kpi,
                "countries_with_data": countries_with_data,
                "countries_matching_filter": len(rows_out),
            }
        else:
            rows_out.sort(key=lambda r: (-(float(r.get("total") or 0)), r.get("country_name") or ""))
        ...
        result: Dict[str, Any] = {
            ...
            "note_platform_region": (
                "Use each row's 'region' for the Operational region column (platform data). "
                "When the user asks for continent, that means operational region — use this field only."
            ),
        }
        result.update(share_meta)
        if share_rows and min_share_pct is not None:
            denom_label = (
                f"total income KPI ({share_meta.get('denominator_kpi_code')})"
                if share_meta.get("denominator_kpi_code")
                else "matrix row sum"
            )
            result["note"] = (
                f"Filtered to {len(rows_out)} of {countries_with_data} countries with data "
                f"(≥{min_share_pct}% share threshold). Percentages use {denom_label} as denominator."
            )
        return result
    except Exception as e:
        logger.error("get_form_field_values_for_all_countries error: %s", e, exc_info=True)
        return service_error(GENERIC_ERROR_MESSAGE, rows=[], count=0)


# FDRS-specific income-sources preset moved to plugins.fdrs.ai_data
# (get_fdrs_income_sources_for_all_countries) — this module stays generic.
