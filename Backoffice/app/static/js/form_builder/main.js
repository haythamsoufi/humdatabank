    ...
    if (sectionForm) {
        sectionForm.addEventListener('submit', async function(e) {
            ...
            try {
                ...
                // The server decodes these before processing.
                if (payload) {
                    ['relevance_condition', 'name_translations'].forEach(function(f) {
                        if (payload[f]) {
                            try {
                                payload[f] = btoa(unescape(encodeURIComponent(String(payload[f]))));
                            } catch (_e) { /* leave unencoded on failure */ }
                        }
                    });
                }
