...
class AIAnswerVerifier:
    ...
    def verify(
        ...
        *,
        ...
        if trace_steps:
            try:
                from app.services.ai.planning.query_intent_helpers import bulk_databank_tool_satisfied
                if bulk_databank_tool_satisfied(trace_steps):
                    return answer, False, []
                # Do not verify databank numeric claims against document chunks when
                # this turn already has structured databank tool results.
                if any(
                    (s or {}).get("action") in {
                ...
                logger.debug("Answer verifier: bulk databank check skipped: %s", exc)
