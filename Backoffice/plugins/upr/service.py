"""Single-dashboard PNG/PDF/InDesign export for UPR."""
...
from plugins.upr.export_job import run_isolated
from plugins.upr.i18n import export_locale, localize_export, translate_styled_blocks
from plugins.upr.raster import render_png_isolated
from plugins.upr.render import render_dashboard_html

logger = logging.getLogger(__name__)

STORAGE_CATEGORY = "upr"
...
class UprVisualsService:
    ...
    def png_bytes(cls, aes_id: int, dashboard_id: str, *, lang: str = "en") -> tuple[bytes, str]:
        ...
        with export_locale(lang):
            work_dir = Path(current_app.instance_path) / "upr_tmp"
    ...
    def idml_zip_bytes(
        ...
        *,
        ...
        with export_locale(lang):
            ...
            work_dir = Path(current_app.instance_path) / "upr_tmp" / f"idml_{uuid.uuid4().hex}"
            work_dir.mkdir(parents=True, exist_ok=True)
        ...
        *,
        ...
        def build() -> tuple[dict[str, Any], str]:
            ...
            if dashboard_id not in DASHBOARD_BY_ID:
                raise UprError(f"Unknown dashboard: {dashboard_id}")
