...
from plugins.upr.errors import UprError, assignment_supports_visuals
from plugins.upr.financial import (
    _plan_financial,
    _report_financial,
    _usable_ifrc_actual,
...
from plugins.upr.indicators import (
    _report_emergencies,
...
from plugins.upr.loaders import (
    _load_aes,
...
from plugins.upr.people_reached import (
...
from plugins.upr.pns_funding import (
...
from plugins.upr.support import (
...
def filename_from_visual_title(title: str, ext: str = "pdf") -> str:
    ...
    raw = (title or "").strip() or "UPR"
    raw = raw.replace("\u2014", " - ").replace("\u2013", "-")
    for char in '<>:"/\\|?*':
        raw = raw.replace(char, " ")
    while ".." in raw:
        raw = raw.replace("..", " ")
    raw = " ".join(raw.split()).strip(" .") or "UPR"
