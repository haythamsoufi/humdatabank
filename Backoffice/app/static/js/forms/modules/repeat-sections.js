...
/**
 * Resolve the live dynamic-indicator section container for a repeat instance.
 * Always query the document at call time so insertions survive DOM moves during
 * repeat-entry creation and layout passes.
 */
function _resolveRepeatIndicatorContainer(sectionId, instanceNumber) {
    if (instanceNumber === 1) {
        return document.getElementById(`section-container-${sectionId}`);
    }
    return document.getElementById(`section-container-${sectionId}-ri-${instanceNumber}`);
}

/**
 * Resolve the "Add indicator" interface element for a repeat instance.
 */
function _resolveRepeatIndicatorInterface(sectionId, instanceNumber) {
    if (instanceNumber === 1) {
        return document.getElementById(`dynamic-indicator-interface-${sectionId}`);
    }
    return document.getElementById(`dynamic-indicator-interface-${sectionId}-ri-${instanceNumber}`);
}
 ...
 * @param {string}        html
 */
function _insertRepeatIndicatorHtml(sectionId, instanceNumber, html) {
    const containerEl = _resolveRepeatIndicatorContainer(sectionId, instanceNumber);
    if (!containerEl) {
        debugWarn('repeat-sections', `Dynamic indicator container not found for section ${sectionId}, instance ${instanceNumber}`);
        return null;
    }
    if (!containerEl.isConnected) {
        debugWarn('repeat-sections', `Dynamic indicator container detached for section ${sectionId}, instance ${instanceNumber}`);
        return null;
    }
    ...
    if (assignmentId) {
        const existing = containerEl.querySelector(`[data-assignment-id="${assignmentId}"]`);
        if (existing) {
            return existing;
        }
    }

    const ifaceEl = _resolveRepeatIndicatorInterface(sectionId, instanceNumber)
        || containerEl.querySelector('[id^="dynamic-indicator-interface-"]');
    const insertionPoint = ifaceEl?.parentElement || containerEl;

    if (ifaceEl && ifaceEl.parentElement) {
        insertionPoint.insertBefore(el, ifaceEl);
    } else {
...
function _finalizeRepeatIndicatorContainer(sectionId, instanceNumber) {
    const containerEl = _resolveRepeatIndicatorContainer(sectionId, instanceNumber);
    if (!containerEl) return;

    initializeFieldListeners(containerEl);

    const repeatEntry = containerEl.closest('.repeat-entry');
    if (repeatEntry && window.addListenersToRepeatEntry) {
        window.addListenersToRepeatEntry(repeatEntry);
    } else if (window.reinitializeDisaggregationCalculator) {
...
function _formatFetchError(err) {
 ...
 *   - If `html` is present it is inserted synchronously (server pre-rendered).
 *   - If `html` is absent the indicator is fetched via the render API (fallback).
 *
...
async function injectExistingRepeatIndicators(sectionId, instanceNumber) {
    ...
    for (const item of items) {
        if (item.html) {
            _insertRepeatIndicatorHtml(sectionId, instanceNumber, item.html);
    ...
    if (lazyItems.length) {
        await Promise.all(lazyItems.map(async (item) => {
            try {
                const html = await _fetchRepeatIndicatorHtml(fetchFn, item.assignment_id);
                _insertRepeatIndicatorHtml(sectionId, instanceNumber, html);
            } catch (err) {
                debugError(
                    'repeat-sections',
                    `Failed to render repeat dynamic indicator ${item.assignment_id}: ${_formatFetchError(err)}`
                );
            }
        }));
    }

    _finalizeRepeatIndicatorContainer(sectionId, instanceNumber);
}
...
function createRepeatEntry(sectionId, isInitialEntry = false) {
    ...
    const pendingDynamicIndicatorLoads = [];
    if (isInitialEntry) {
        ...
        dynSubSections.forEach(subSection => {
            ...
            pendingDynamicIndicatorLoads.push({ sectionId: subSectionId, instanceNumber: 1 });
        });
    } else {
        ...
        if (entry1) {
            ...
            dynSubSections1.forEach(refSection => {
                ...
                pendingDynamicIndicatorLoads.push({ sectionId: subSectionId, instanceNumber });
    ...
    // Load saved dynamic indicators only after the repeat entry is in the live DOM.
    pendingDynamicIndicatorLoads.forEach(({ sectionId: subSectionId, instanceNumber: instNum }) => {
        injectExistingRepeatIndicators(subSectionId, instNum);
    });
