...
def test_collect_narrative_uploads_from_zip_and_pdf():
    ...
    assert "bangladesh" in found
    with pytest.raises(UprError, match="Word documents"):
...
def test_run_export_job_file_pdf(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("plugins.upr.export_job.render_pdf_bytes", fake_pdf)
...
def test_run_export_job_file_png_from_pdf(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("plugins.upr.export_job.render_png_from_pdf", fake_from_pdf)
    run_export_job_file(job)
...
def test_run_export_job_file_idml(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("plugins.upr.export_job.render_pdf_bytes", lambda *_a, **_k: b"%PDF")

    def fake_package(**_kw):
        return {"zip_bytes": b"PK-idml"}

    monkeypatch.setattr("plugins.upr.idml.build_indesign_package", fake_package)
    run_export_job_file(job)
...
def test_run_export_job_file_narrative_uses_pretranslated_blocks(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("plugins.upr.export_job.render_pdf_bytes", lambda *_a, **_k: b"%PDF-vis")

    def fake_narrative(blocks, folio=""):
        assert blocks[0]["text"] == "مرحبا"
        return b"%PDF-nar"

    monkeypatch.setattr("plugins.upr.idml.render_narrative_pdf_bytes", fake_narrative)
    monkeypatch.setattr(
        "plugins.upr.idml.merge_report_pdfs",
...
def test_run_export_job_file_narrative_restyles_attached_pdf(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("plugins.upr.export_job.render_pdf_bytes", lambda *_a, **_k: b"%PDF-vis")
    monkeypatch.setattr(
        "plugins.upr.idml.load_narrative_paragraphs",
        lambda data: [{"text": "Hello from PDF"}] if data.startswith(b"%PDF") else [],
    )
    monkeypatch.setattr(
        "plugins.upr.idml.style_narrative_blocks",
        lambda blocks, country_name="": [{"style": "Body", "text": blocks[0]["text"]}],
    )
    monkeypatch.setattr(
        "plugins.upr.i18n.translate_styled_blocks",
