"""UPR assignment and bulk-admin HTTP routes."""
...
from plugins.upr.routes import MAX_BULK_AES_IDS, MAX_BULK_DASHBOARDS


@pytest.mark.unit
def test_settings_page_route_is_registered(app):
    endpoint, values = app.url_map.bind("localhost").match("/admin/plugins/upr/settings")
    assert endpoint == "upr.settings_page"
    assert values == {}


@pytest.mark.unit
def test_settings_page_requires_login(client):
    response = client.get("/admin/plugins/upr/settings")
    assert response.status_code in (302, 401)


@pytest.mark.unit
def test_settings_page_renders_for_system_manager(logged_in_sm_client):
    response = logged_in_sm_client.get("/admin/plugins/upr/settings")
...
def test_visuals_progress_route_is_registered(app):
    ...
    assert endpoint == "upr.assignment_visuals_progress"
...
def test_live_assignment_pdf_route_is_registered(app):
    ...
    assert endpoint == "upr.assignment_pdf"
    assert values == {"aes_id": 42}
...
def test_dashboard_assignment_pdf_route_is_registered(app):
    ...
    assert endpoint == "upr.assignment_pdf_dashboard"
...
def test_bulk_routes_require_login(client):
    assert client.get("/admin/data-exploration/upr/assignments").status_code in (302, 401)
    assert client.get("/admin/data-exploration/upr/status").status_code in (302, 401)
    assert client.post("/admin/data-exploration/upr/generate", json={}).status_code in (302, 401)
    assert client.post("/admin/data-exploration/upr/cancel", json={"job_id": "x"}).status_code in (302, 401)
    assert client.get("/admin/data-exploration/upr/download/job-x").status_code in (302, 401)


@pytest.mark.unit
def test_bulk_routes_forbid_non_system_manager(logged_in_focal_client):
    resp = logged_in_focal_client.get("/admin/data-exploration/upr/assignments")
    assert resp.status_code in (302, 403)
    resp = logged_in_focal_client.get("/admin/data-exploration/upr/status")
    assert resp.status_code in (302, 403)
...
def test_generate_rejects_oversized_country_and_dashboard_lists(logged_in_sm_client):
    too_many_aes = logged_in_sm_client.post(
        "/admin/data-exploration/upr/generate",
        json={
            "assigned_form_id": 1,
...
def test_generate_status_cancel_download_flow(logged_in_sm_client, monkeypatch):
    ...
    status = logged_in_sm_client.get("/admin/data-exploration/upr/status?job_id=job-flow")
    assert status.status_code == 200
    assert status.get_json()["status"]["job_id"] == "job-flow"

    cancelled = logged_in_sm_client.post(
        "/admin/data-exploration/upr/cancel",
        json={"job_id": "job-flow"},
    )
    assert cancelled.status_code == 200
    assert cancelled.get_json()["cancelled"] is True

    download = logged_in_sm_client.get("/admin/data-exploration/upr/download/job-flow")
...
def test_narrative_route_starts_background_job(logged_in_sm_client, monkeypatch):
    started = []
    monkeypatch.setattr(routes, "_aes_or_404", lambda _aes_id: object())
    monkeypatch.setattr(routes, "read_narrative_upload", lambda upload, filename="": b"PK")
