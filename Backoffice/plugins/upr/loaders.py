...
def _load_aes(aes_id: int) -> AssignmentEntityStatus:
    ...
    if not aes:
        raise UprError("Assignment not found.")
...
def _load_dynamic_indicator_rows(aes_id: int) -> list[DynamicIndicatorData]:
    from plugins.upr.i18n import memoized_load
...
def _load_dynamic_indicator_rows_db(aes_id: int) -> list[DynamicIndicatorData]:
    ...
    try:
        ...
        db.session.rollback()
        logger.debug("UPR: falling back to dynamic rows without SPEF catalog join", exc_info=True)
        return (
...
def _load_items(template) -> list[FormItem]:
    ...
    try:
        ...
        db.session.rollback()
        logger.debug("UPR: falling back to items without SPEF catalog join", exc_info=True)
        return (
