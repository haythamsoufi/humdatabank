"""
Comprehensive tests for app/routes/admin/template_special.py
...
def _auth():
    ...
    with patch("app.routes.admin.shared.AuthorizationService.is_admin", return_value=True), \
         ...
         patch("app.routes.admin.template_special.check_template_access", return_value=True):
        yield


def _mock_render(return_value="<html>template</html>"):
    return patch(
        "app.routes.admin.template_special.render_template",
        return_value=return_value,
    )


# ---------------------------------------------------------------------------
# GET /<template_id>  –  special_template_view
# ---------------------------------------------------------------------------
...
class TestSpecialTemplateView:
    def test_get_existing_template_renders(self, logged_in_client, db_session, app):
        ...
        with _auth(), _mock_render() as mock_rt:
            resp = logged_in_client.get(f"/admin/templates/special/{template.id}")
        assert resp.status_code == 200
        mock_rt.assert_called_once()

    def test_get_nonexistent_template_returns_404(self, logged_in_client, db_session):
        with _auth():
            resp = logged_in_client.get("/admin/templates/special/99999")
    ...
    def test_get_template_access_denied_redirects(self, logged_in_client, db_session, app):
        ...
        with patch("app.routes.admin.shared.AuthorizationService.is_admin", return_value=True), \
             patch("app.routes.admin.shared.AuthorizationService.has_rbac_permission", return_value=True), \
             patch("app.routes.admin.template_special.check_template_access", return_value=False):
            resp = logged_in_client.get(
                f"/admin/templates/special/{template.id}",
                follow_redirects=False,
            )
        assert resp.status_code in (200, 302)

    def test_get_unauthenticated_redirects(self, client, db_session):
        resp = client.get("/admin/templates/special/1", follow_redirects=False)
...
class TestImputeTemplate2:
    def test_missing_target_period_redirects(self, logged_in_client, db_session, app):
        with _auth():
            resp = logged_in_client.post(
                "/admin/templates/special/impute/template2",
                data={},
                follow_redirects=False,
            )
        assert resp.status_code == 302
    ...
    def test_valid_period_success(self, logged_in_client, db_session, app):
        ...
        with _auth(), \
             patch("app.routes.admin.template_special.ImputationService.impute_template_2",
    ...
    def test_unauthenticated_redirects(self, client, db_session):
        resp = client.post("/admin/templates/special/impute/template2", follow_redirects=False)
        assert resp.status_code == 302
...
class TestUpdateImputationMethodsBatch:
    def test_no_updates_returns_400(self, logged_in_client, db_session, app):
        with _auth():
            resp = logged_in_client.post(
                "/admin/templates/special/update-imputation-methods-batch",
                json={"updates": []},
                headers=_json_headers(),
            )
    ...
    def test_unauthenticated_redirects(self, client, db_session):
        resp = client.post("/admin/templates/special/update-imputation-methods-batch", follow_redirects=False)
        assert resp.status_code == 302
...
class TestPreviewData:
    def test_missing_year_returns_400(self, logged_in_client, db_session, app):
        ...
        with _auth():
            resp = logged_in_client.get(f"/admin/templates/special/{template.id}/preview-data")
        assert resp.status_code == 400
    ...
    def test_with_year_no_assignments(self, logged_in_client, db_session, app):
        ...
        with _auth():
            resp = logged_in_client.get(
                f"/admin/templates/special/{template.id}/preview-data?year=2025"
            )
        assert resp.status_code == 200
        data = resp.get_json()
    ...
    def test_nonexistent_template_returns_404(self, logged_in_client, db_session):
        with _auth():
            resp = logged_in_client.get("/admin/templates/special/99999/preview-data?year=2025")
        assert resp.status_code == 404

    def test_unauthenticated_redirects(self, client, db_session):
        resp = client.get("/admin/templates/special/1/preview-data?year=2025", follow_redirects=False)
        assert resp.status_code == 302
...
class TestPreviewImputation:
    def test_missing_year_returns_400(self, logged_in_client, db_session, app):
        ...
        with _auth():
            resp = logged_in_client.post(
                f"/admin/templates/special/{template.id}/preview-imputation",
                json={},
                headers=_json_headers(),
            )
        assert resp.status_code == 400
    ...
    def test_nonexistent_template_returns_404(self, logged_in_client, db_session):
        with _auth():
            resp = logged_in_client.post(
                "/admin/templates/special/99999/preview-imputation",
                json={"year": "2025"},
                headers=_json_headers(),
            )
        assert resp.status_code == 404
    ...
    def test_unauthenticated_redirects(self, client, db_session):
        resp = client.post("/admin/templates/special/1/preview-imputation", follow_redirects=False)
        assert resp.status_code == 302
...
class TestPreviewDataChunked:
    def test_missing_year_returns_400(self, logged_in_client, db_session, app):
        ...
        with _auth():
            resp = logged_in_client.post(
                f"/admin/templates/special/{template.id}/preview-data-chunked",
                json={},
                headers=_json_headers(),
            )
        assert resp.status_code == 400
    ...
    def test_nonexistent_template_returns_404(self, logged_in_client, db_session):
        with _auth():
            resp = logged_in_client.post(
                "/admin/templates/special/99999/preview-data-chunked",
                json={"year": "2025"},
                headers=_json_headers(),
            )
        assert resp.status_code == 404
...
class TestPreviewImputationChunked:
    def test_missing_year_returns_400(self, logged_in_client, db_session, app):
        ...
        with _auth():
            resp = logged_in_client.post(
                f"/admin/templates/special/{template.id}/preview-imputation-chunked",
                json={},
                headers=_json_headers(),
            )
        assert resp.status_code == 400
    ...
    def test_nonexistent_template_returns_404(self, logged_in_client, db_session):
        with _auth():
            resp = logged_in_client.post(
                "/admin/templates/special/99999/preview-imputation-chunked",
                json={"year": "2025"},
                headers=_json_headers(),
            )
        assert resp.status_code == 404
...
class TestFilterOptions:
    def test_missing_year_returns_400(self, logged_in_client, db_session, app):
        ...
        with _auth():
            resp = logged_in_client.get(f"/admin/templates/special/{template.id}/filter-options")
        assert resp.status_code == 400
    ...
    def test_with_year_returns_ok(self, logged_in_client, db_session, app):
        ...
        with _auth():
            resp = logged_in_client.get(
                f"/admin/templates/special/{template.id}/filter-options?year=2025"
            )
        assert resp.status_code == 200
        data = resp.get_json()
    ...
    def test_nonexistent_template_returns_404(self, logged_in_client, db_session):
        with _auth():
            resp = logged_in_client.get("/admin/templates/special/99999/filter-options?year=2025")
        assert resp.status_code == 404

    def test_unauthenticated_redirects(self, client, db_session):
        resp = client.get("/admin/templates/special/1/filter-options?year=2025", follow_redirects=False)
        assert resp.status_code == 302


# ---------------------------------------------------------------------------
# GET /<template_id>/max-year
# ---------------------------------------------------------------------------

class TestMaxYear:
    def test_max_year_no_assignments_returns_current_year(self, logged_in_client, db_session, app):
        template = create_test_template(db_session, name="Max Year Template")
        with _auth():
            resp = logged_in_client.get(f"/admin/templates/special/{template.id}/max-year")
        assert resp.status_code == 200
        data = resp.get_json()
        assert data.get("success") is True or "max_year" in data

    def test_max_year_with_assignments(self, logged_in_client, db_session, app):
        from tests.factories import create_test_assignment_entity_status
        template = create_test_template(db_session, name="Max Year With Assign Template")
        country = create_test_country(db_session, name="Max Year Country", iso3="MYC", iso2="MY")
    ...
    def test_nonexistent_template_returns_404(self, logged_in_client, db_session):
        with _auth():
            resp = logged_in_client.get("/admin/templates/special/99999/max-year")
        assert resp.status_code == 404

    def test_unauthenticated_redirects(self, client, db_session):
        resp = client.get("/admin/templates/special/1/max-year", follow_redirects=False)
        assert resp.status_code == 302
...
class TestRunImputationFiltered:
    def test_missing_year_returns_400(self, logged_in_client, db_session, app):
        ...
        with _auth():
            resp = logged_in_client.post(
                f"/admin/templates/special/{template.id}/run-imputation-filtered",
                json={},
                headers=_json_headers(),
            )
        assert resp.status_code == 400
    ...
    def test_template2_imputation_success(self, logged_in_client, db_session, app):
        ...
        with _auth(), \
             patch("app.routes.admin.template_special.ImputationService.impute_template_2_filtered",
                   return_value=mock_result):
            # Force template_id to 2 by patching the route check
            resp = logged_in_client.post(
                "/admin/templates/special/2/run-imputation-filtered",
                json={"year": "2025"},
                headers=_json_headers(),
            )
    ...
    def test_nonexistent_template_returns_404(self, logged_in_client, db_session):
        with _auth():
            resp = logged_in_client.post(
                "/admin/templates/special/99999/run-imputation-filtered",
                json={"year": "2025"},
                headers=_json_headers(),
            )
        assert resp.status_code == 404
    ...
    def test_unauthenticated_redirects(self, client, db_session):
        resp = client.post("/admin/templates/special/1/run-imputation-filtered", follow_redirects=False)
        assert resp.status_code == 302
...
class TestExportPreviewExcel:
    ...
    def test_missing_year_or_data_returns_400(self, logged_in_client, db_session, app):
        ...
        with _auth():
            resp = logged_in_client.post(
                f"/admin/templates/special/{template.id}/export-excel",
                json={"year": "2025"},
                headers=_json_headers(),
            )
        assert resp.status_code == 400
    ...
    def test_unauthenticated_redirects(self, client, db_session):
        resp = client.post("/admin/templates/special/1/export-excel", follow_redirects=False)
        assert resp.status_code == 302


# ---------------------------------------------------------------------------
# POST /<template_id>/run-fdrs-sync
# ---------------------------------------------------------------------------

class TestRunFdrsSync:
    def _post_fdrs_sync(self, logged_in_client, template_id, data=None):
        defaults = {"dry_run": False}
        if data:
            defaults.update(data)
        return logged_in_client.post(
            f"/admin/templates/special/{template_id}/run-fdrs-sync",
            json=defaults,
            headers=_json_headers(),
        )
    ...
    def test_nonexistent_template_returns_404(self, logged_in_client, db_session):
        with _auth():
            resp = self._post_fdrs_sync(logged_in_client, 99999)
        assert resp.status_code == 404
    ...
    def test_access_denied_returns_403(self, logged_in_client, db_session, app):
        ...
        with patch("app.routes.admin.shared.AuthorizationService.is_admin", return_value=True), \
             patch("app.routes.admin.shared.AuthorizationService.has_rbac_permission", return_value=True), \
             patch("app.routes.admin.template_special.check_template_access", return_value=False):
            resp = self._post_fdrs_sync(logged_in_client, template.id)
    ...
    def test_invalid_batch_size_returns_400(self, logged_in_client, db_session, app):
        ...
        with _auth():
            resp = self._post_fdrs_sync(logged_in_client, template.id, {"batch_size": "abc"})
        assert resp.status_code == 400
    ...
    def test_batch_size_too_small_returns_400(self, logged_in_client, db_session, app):
        ...
        with _auth():
            resp = self._post_fdrs_sync(logged_in_client, template.id, {"batch_size": 50})
        assert resp.status_code == 400
    ...
    def test_test_mode_limits_data(self, logged_in_client, db_session, app):
        ...
        with _auth(), \
             patch.dict("sys.modules", {"import_fdrs_form_data": MagicMock(run_import=mock_run_import)}):
            resp = self._post_fdrs_sync(logged_in_client, template.id, {"test": True})
        assert resp.status_code in (200, 400, 500)
    ...
    def test_dry_run_with_import(self, logged_in_client, db_session, app):
        ...
        with _auth(), \
             patch.dict("sys.modules", {"import_fdrs_form_data": MagicMock(run_import=mock_run_import)}):
            resp = self._post_fdrs_sync(logged_in_client, template.id, {"dry_run": True})
        assert resp.status_code in (200, 400, 500)

    def test_unauthenticated_redirects(self, client, db_session):
        resp = client.post("/admin/templates/special/1/run-fdrs-sync", follow_redirects=False)
        assert resp.status_code == 302
...
# ---------------------------------------------------------------------------
# GET /<template_id>/fdrs-sync-status/<job_id>
# ---------------------------------------------------------------------------

class TestFdrsSyncStatus:
    def _inject_job(self, template_id, user_id, status="running"):
        from app.routes.admin.template_special import _FDRS_SYNC_JOBS, _FDRS_SYNC_LOCK
    ...
    def test_nonexistent_job_returns_404(self, logged_in_client, db_session, app):
        ...
        with _auth():
            resp = logged_in_client.get(
                f"/admin/templates/special/{template.id}/fdrs-sync-status/nonexistent_job"
            )
        assert resp.status_code == 404
    ...
    def test_job_wrong_template_returns_404(self, logged_in_client, db_session, app):
        ...
        with _auth():
            resp = logged_in_client.get(
                f"/admin/templates/special/{template1.id}/fdrs-sync-status/{job_id}"
            )
        assert resp.status_code == 404
    ...
    def test_job_different_user_returns_403(self, logged_in_client, db_session, app, admin_user):
        ...
        with _auth():
            resp = logged_in_client.get(
                f"/admin/templates/special/{template.id}/fdrs-sync-status/{job_id}"
            )
        assert resp.status_code == 403
    ...
    def test_job_with_download_ready_includes_url(self, logged_in_client, db_session, app, admin_user):
        ...
        with _FDRS_SYNC_LOCK:
            _FDRS_SYNC_JOBS[job_id]["download_ready"] = True
        with _auth():
            resp = logged_in_client.get(
                f"/admin/templates/special/{template.id}/fdrs-sync-status/{job_id}"
            )
    ...
    def test_unauthenticated_redirects(self, client, db_session):
        resp = client.get("/admin/templates/special/1/fdrs-sync-status/fakejob", follow_redirects=False)
        assert resp.status_code == 302


# ---------------------------------------------------------------------------
# POST /<template_id>/fdrs-sync-cancel/<job_id>
# ---------------------------------------------------------------------------

class TestFdrsSyncCancel:
    def _inject_job(self, template_id, user_id, status="running"):
        from app.routes.admin.template_special import _FDRS_SYNC_JOBS, _FDRS_SYNC_LOCK
        job_id = uuid.uuid4().hex
        with _FDRS_SYNC_LOCK:
    ...
    def test_nonexistent_job_returns_404(self, logged_in_client, db_session, app):
        ...
        with _auth():
            resp = logged_in_client.post(
                f"/admin/templates/special/{template.id}/fdrs-sync-cancel/nonexistent_job"
            )
        assert resp.status_code == 404
    ...
    def test_wrong_user_returns_403(self, logged_in_client, db_session, app, admin_user):
        ...
        with _auth():
            resp = logged_in_client.post(
                f"/admin/templates/special/{template.id}/fdrs-sync-cancel/{job_id}"
            )
        assert resp.status_code == 403
    ...
    def test_unauthenticated_redirects(self, client, db_session):
        resp = client.post("/admin/templates/special/1/fdrs-sync-cancel/fakejob", follow_redirects=False)
        assert resp.status_code == 302


# ---------------------------------------------------------------------------
# GET /<template_id>/fdrs-sync-download/<job_id>
# ---------------------------------------------------------------------------

class TestFdrsSyncDownload:
    ...
    def test_nonexistent_job_returns_404(self, logged_in_client, db_session, app):
        ...
        with _auth():
            resp = logged_in_client.get(
                f"/admin/templates/special/{template.id}/fdrs-sync-download/nonexistent_job"
            )
        assert resp.status_code == 404
    ...
    def test_wrong_user_returns_403(self, logged_in_client, db_session, app, admin_user):
        ...
        with _auth():
            resp = logged_in_client.get(
                f"/admin/templates/special/{template.id}/fdrs-sync-download/{job_id}"
            )
        assert resp.status_code == 403
    ...
    def test_unauthenticated_redirects(self, client, db_session):
        resp = client.get("/admin/templates/special/1/fdrs-sync-download/fakejob", follow_redirects=False)
        assert resp.status_code == 302
...
class TestInternalHelpers:
    def test_utc_iso_returns_string(self):
        from app.routes.admin.template_special import _utc_iso
        result = _utc_iso()
        assert isinstance(result, str)
        assert "T" in result

    def test_cleanup_fdrs_jobs_locked_removes_old_jobs(self):
        from app.routes.admin.template_special import (
            _FDRS_SYNC_JOBS, _FDRS_SYNC_LOCK, _FDRS_SYNC_TTL_SECONDS,
            _cleanup_fdrs_jobs_locked
        )
        old_ts = time.time() - _FDRS_SYNC_TTL_SECONDS - 100
        job_id = uuid.uuid4().hex
        with _FDRS_SYNC_LOCK:
            _FDRS_SYNC_JOBS[job_id] = {
                "job_id": job_id,
        ...
        assert job_id not in _FDRS_SYNC_JOBS

    def test_cleanup_fdrs_jobs_locked_preserves_fresh_jobs(self):
        from app.routes.admin.template_special import (
            _FDRS_SYNC_JOBS, _FDRS_SYNC_LOCK, _cleanup_fdrs_jobs_locked
        )
        job_id = uuid.uuid4().hex
        with _FDRS_SYNC_LOCK:
            _FDRS_SYNC_JOBS[job_id] = {
            ...
            _cleanup_fdrs_jobs_locked(time.time())
        assert job_id in _FDRS_SYNC_JOBS
        # Clean up
        with _FDRS_SYNC_LOCK:
            _FDRS_SYNC_JOBS.pop(job_id, None)

    def test_get_fdrs_sync_cancel_event_creates_event(self):
        from app.routes.admin.template_special import _get_fdrs_sync_cancel_event
        import threading
        job_id = uuid.uuid4().hex
        event = _get_fdrs_sync_cancel_event(job_id)
        assert isinstance(event, threading.Event)
        from app.routes.admin.template_special import _clear_fdrs_sync_cancel_event
        _clear_fdrs_sync_cancel_event(job_id)

    def test_get_fdrs_sync_cancel_event_returns_same_event(self):
        from app.routes.admin.template_special import _get_fdrs_sync_cancel_event, _clear_fdrs_sync_cancel_event
        job_id = uuid.uuid4().hex
        e1 = _get_fdrs_sync_cancel_event(job_id)
        e2 = _get_fdrs_sync_cancel_event(job_id)
        assert e1 is e2
        _clear_fdrs_sync_cancel_event(job_id)

    def test_parse_fdrs_reported_import_states_none_key(self):
        from app.routes.admin.template_special import _parse_fdrs_reported_import_states_from_request
        result = _parse_fdrs_reported_import_states_from_request({})
        assert result is None

    def test_parse_fdrs_reported_import_states_none_value(self):
        from app.routes.admin.template_special import _parse_fdrs_reported_import_states_from_request
        result = _parse_fdrs_reported_import_states_from_request({"fdrs_reported_import_states": None})
        assert result is None

    def test_parse_fdrs_reported_import_states_list(self):
        from app.routes.admin.template_special import _parse_fdrs_reported_import_states_from_request
        result = _parse_fdrs_reported_import_states_from_request({"fdrs_reported_import_states": [100, 200]})
        assert result == [100, 200]

    def test_parse_fdrs_reported_import_states_string(self):
        from app.routes.admin.template_special import _parse_fdrs_reported_import_states_from_request
        result = _parse_fdrs_reported_import_states_from_request({"fdrs_reported_import_states": "100,200"})
        assert result == [100, 200]

    def test_parse_fdrs_reported_import_states_invalid_value_raises(self):
        from app.routes.admin.template_special import _parse_fdrs_reported_import_states_from_request
        with pytest.raises(ValueError):
            _parse_fdrs_reported_import_states_from_request({"fdrs_reported_import_states": [999]})

    def test_parse_fdrs_reported_import_states_empty_list_raises(self):
        from app.routes.admin.template_special import _parse_fdrs_reported_import_states_from_request
        with pytest.raises(ValueError):
            _parse_fdrs_reported_import_states_from_request({"fdrs_reported_import_states": []})

    def test_parse_fdrs_reported_import_states_invalid_string_raises(self):
        from app.routes.admin.template_special import _parse_fdrs_reported_import_states_from_request
        with pytest.raises(ValueError):
            _parse_fdrs_reported_import_states_from_request({"fdrs_reported_import_states": "notanumber"})

    def test_parse_fdrs_reported_import_states_invalid_list_item_raises(self):
        from app.routes.admin.template_special import _parse_fdrs_reported_import_states_from_request
        with pytest.raises(ValueError):
            _parse_fdrs_reported_import_states_from_request({"fdrs_reported_import_states": ["notanumber"]})

    def test_parse_fdrs_reported_import_states_wrong_type_raises(self):
        from app.routes.admin.template_special import _parse_fdrs_reported_import_states_from_request
        with pytest.raises(ValueError):
            _parse_fdrs_reported_import_states_from_request({"fdrs_reported_import_states": 123})

    def test_build_ordered_sections_with_items_empty(self, app, db_session):
        with app.app_context():
            from app.routes.admin.template_special import _build_ordered_sections_with_items
