...
def manage_settings():
    ...
    if request.method == "POST":
        ...
        # The orchestrator JS posts ``{"payload": "<b64 of inner JSON>"}`` to dodge WAF
        # false positives. ``get_request_data()`` already unwraps that envelope, so ``data``
        # here is the inner dict (or ``request.form`` for non-JSON submissions).
        #
        # Two submission shapes are supported:
        #   * Full save: every form key the page renders is present.
        #   * Partial save: only changed keys + ``settings_partial_save="1"`` are sent;
        #     we merge them on top of a baseline mirroring the rendered form so that
        #     unconditional sections below (languages, document types, audience rules…)
        #     don't overwrite untouched settings with empty values.
        ...
        is_json_submission = isinstance(data, _JsonFormProxy)
        partial_flag = is_json_submission and _truthy_flag(data.get("settings_partial_save"))

        try:
            if is_json_submission:
                _audit_keys = [
                    k for k in data.keys()
                    if k and k not in ("csrf_token", "settings_partial_save")
                ]
            else:
                _audit_keys = [k for k in (data.keys() or []) if k and k != "csrf_token"]
            settings_audit_detail = {"partial": partial_flag, "keys": list(_audit_keys)}
        except Exception:
            current_app.logger.debug("manage_settings: failed to capture audit keys", exc_info=True)
            settings_audit_detail = None

        if partial_flag:
            try:
                inner = data.to_dict()
                inner.pop("settings_partial_save", None)
                token = inner.get("csrf_token") or ""
                ...
                merged = {**baseline, **inner}
                data = _JsonFormProxy(merged)
            except Exception:
                current_app.logger.exception("manage_settings: failed to merge partial-save payload")
        ...
        try:
            ...
            try:
                ...
                current_app.logger.exception("set_ai_beta_access_settings failed")
                ai_beta_ok = False
            ...
            current_app.logger.exception("manage_settings: unhandled error while saving")
            flash("An error occurred. Please try again.", "danger")
