...
| Login works, first country/entity pick returns to `/login` with “session needed a refresh / changes were not saved” (phones) | CSRF failed on the context-switch POST *and* the handler wrote a new anonymous session cookie, or the login `Set-Cookie` was on a 302 that ITP discarded | Context switch is GET (no CSRF). Login uses a first-party 200 continue page so the cookie sticks. Unauthenticated CSRF failures must not emit a replacement session cookie. `WTF_CSRF_SSL_STRICT` is off — phones often omit Referer. |
