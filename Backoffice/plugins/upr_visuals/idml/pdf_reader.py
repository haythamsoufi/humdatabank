...
from plugins.upr_visuals.idml.word_reader import MAX_NARRATIVE_BLOCKS, safe_export_href
        ...
        for page in doc:
            ...
            for _y, parsed in items:
                ...
                if len(blocks) > MAX_NARRATIVE_BLOCKS:
                    raise UprVisualsError("The narrative document has too many paragraphs.")
        if not any((row.get("text") or "").strip() or row.get("kind") == "table" for row in blocks):
            raise UprVisualsError(
