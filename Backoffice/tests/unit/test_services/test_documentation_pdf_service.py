...
from app.services.documentation.pdf_service import (
...
def test_build_pdf_branding_context_uses_localized_org_name(app):
    with app.app_context():
        with patch(
            "app.services.documentation.pdf_service.get_org_name",
            side_effect=lambda locale=None, **kwargs: "Acme NS" if locale == "fr" else "Acme",
        ), patch(
            "app.services.documentation.pdf_service._resolve_pdf_header_logo_uri",
            return_value=None,
        ):
...
def test_is_rtl_for_arabic_and_persian():
    from app.services.documentation.pdf_service import _is_rtl
...
def test_pdf_css_includes_tajawal_and_rtl_rules(app):
    from app.services.documentation.pdf_service import _pdf_css
