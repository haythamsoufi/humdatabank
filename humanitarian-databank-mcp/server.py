...
mcp = FastMCP(
    ...
    instructions=(
        ...
        "For global totals by year (e.g. volunteers across all countries), prefer "
        "databank_aggregate_global_trend(query='volunteers') — it dedupes submissions "
        "and returns compact period totals. Use databank_resolve_indicator to map "
        "natural-language metric names to indicator IDs. Use databank_get_public_data* "
        "only when you need raw rows or country-level detail; set include_dimensions=true "
        "only if you need join tables (countries, indicator_bank, etc.)."
    ),
)
...
def databank_resolve_indicator(query: str, limit: int = 5) -> str:
    ...
    Handles canonical headcount metrics (volunteers → id 724 / KPI_PeopleVol,
    staff, branches, local units) and ranked search for other indicators.
    Prefer this before broad databank_search_indicators calls.
    """
    try:
        return _json_text(resolve_indicator(query, limit=limit))
...
def databank_aggregate_global_trend(
    ...
    multiple submissions per country+period (keeps latest submission_id). Only
    includes public data with data_status='available'. Returns compact by_period
    totals — do not sum raw databank_get_public_data rows without deduping.
    """
    try:
        result = aggregate_global_trend(
...
def databank_get_public_data(
    ...
    only (dimension tables stripped). Set include_dimensions=true for countries,
    indicator_bank, form_items, assignment_statuses. Multiple submissions may
    exist per country+period — use databank_aggregate_global_trend for totals.
    """
    try:
