...
LLM execution router: decides full ReAct agent vs one-shot fast path.
Fast path runs only when the LLM explicitly sets execution_mode=\"fast_path\"
with high confidence — see ``AgentRoutingPolicy.decide_fast_path_plan``.
"""
...
class AIQueryPlanner:
    ...
    def _validate_simple_plan_dict(plan: Dict[str, Any], tool_names: Set[str]) -> Optional[SimplePlan]:
        if not isinstance(plan, dict):
            return None
        execution_mode = str(plan.get("execution_mode") or "full_agent").strip().lower()
        if execution_mode != "fast_path":
            logger.info(
                "Query planner: LLM chose execution_mode=%s — using full ReAct agent",
                execution_mode or "full_agent",
            )
            return None
        ...
        try:
            ...
            confidence = 0.0
        min_conf = float(current_app.config.get("AI_AGENT_FAST_PATH_MIN_CONFIDENCE", 0.85))
        if confidence < min_conf:
            logger.info(
                "Query planner: fast_path confidence %.2f below threshold %.2f — full ReAct",
                confidence,
                min_conf,
            )
            return None
        ...
        required_by_tool = {
            ...
            "get_form_field_values_for_all_countries": {"field_label_or_name"},
    ...
    def plan_simple(
        ...
        *,
        ...
        sys = (
            "You are the execution router for a data assistant.\n"
            "You decide whether to run a one-shot fast path OR the full multi-step ReAct agent.\n"
            "Return ONLY valid JSON with shape:\n"
            "{"
            "\"execution_mode\": \"full_agent\"|\"fast_path\", "
            ...
            "}\n"
            "Rules:\n"
            "- DEFAULT execution_mode=\"full_agent\" — use this whenever unsure.\n"
            "- Use execution_mode=\"fast_path\" ONLY for standalone first-turn questions that need "
            "exactly ONE tool call, with confidence >= 0.85, and NO reliance on prior conversation.\n"
            "- ALWAYS execution_mode=\"full_agent\" when recent_conversation is present and the current "
            "message is a follow-up (e.g. 'what about', 'why only', 'and those', 'same but', pronouns, "
            "or a changed threshold on the same topic).\n"
            "- ALWAYS execution_mode=\"full_agent\" for form matrix share / FDRS income-source / "
            "percentage-threshold questions — those need get_form_field_values_for_all_countries in "
            "the full agent, NOT search_documents fast path.\n"
            "- If unsure, set execution_mode=\"full_agent\", is_simple=false, kind='complex'.\n"
            ...
            "- **Form matrix / FDRS income share questions** (e.g. countries where X% of income comes from "
            "individuals, home/foreign government, etc.): use get_form_field_values_for_all_countries with "
            "field_label_or_name='income sources', template_identifier='FDRS', matrix_share_rows (row labels "
            "such as 'Individuals', 'Home Government'), min_share_pct, and share_match. NEVER use search_documents "
            "for structured FDRS income-source breakdowns — that data lives in submitted form matrix values.\n"
        ...
        user_payload: Dict[str, Any] = {"query": q, "allowed_tools": allowed}
        ...
        try:
            ...
            validated = self._validate_simple_plan_dict(obj or {}, tool_names=tool_names) if obj else None
            if validated is not None:
                logger.info(
                    "Query planner: LLM opted into fast_path (tool=%s, kind=%s)",
                    validated.tool_name,
                    validated.kind,
                )
            return validated
